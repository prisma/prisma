...
- **CLI commands**: Most commands already accept `--config` for custom config paths. Upcoming work removes `--schema` / `--url` in favour of config-based resolution. When editing CLI help text, keep examples aligned with new config-first workflow.
  ...
  - `packages/cli/src/Studio.ts` now serves only explicit Studio assets (`/`, `/favicon.ico`, `/studio.js`, `/studio.css`, `/bff`, `/telemetry`) through a plain `Request -> Response` handler, and runtime-specific listeners live in `packages/cli/src/studio-server.ts` rather than Hono.
  - Keep binding-level coverage for the Studio server: a previous Node adapter regression dropped all `GET` response bodies because it treated `GET` like `HEAD`, so `packages/cli/src/__tests__/studio-server.vitest.ts` exists to verify real listener behavior in addition to handler-only tests.
