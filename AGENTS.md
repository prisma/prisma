...
- **Client architecture (Prisma 7)**:
  ...
  - `client-engine-cache-timing.ts` also has render-only, data-map-only, and nested root-data-map phase rows for cached plans. Current rows show `renderQuery()` for the first DB query and scalar data mapping are small relative to nested cached-plan execution; the nested split shows the root `dataMap` plan shape is materially slower than a manual inner-expression plus outer-map loop, so inspect plan-shape/interpreter effects before more render-only tuning.
