...
- **Driver adapters & runtimes**: `packages/bundled-js-drivers` plus the `adapter-*` packages ship JS driver adapters: `adapter-pg`, `adapter-bun-postgres`, `adapter-neon`, `adapter-libsql`, `adapter-planetscale`, `adapter-d1`, `adapter-better-sqlite3`, `adapter-mssql`, `adapter-mariadb`, `adapter-ppg` (Prisma Postgres Serverless). These are built on helpers in `driver-adapter-utils`; migrate/client fixtures exercise them, so adapter changes typically require fixture/test updates. CI tests driver adapters with flavors: `js_pg`, `js_bun_postgres`, `js_neon`, `js_libsql`, `js_planetscale`, `js_d1`, `js_better_sqlite3`, `js_mssql`, `js_mariadb`, `js_pg_cockroachdb`.
...
- **Client functional tests structure**:
  ...
  - Run specific adapter: `pnpm --filter @prisma/client test:functional:code --adapter js_pg <pattern>` (adapters: `js_pg`, `js_bun_postgres`, `js_neon`, `js_libsql`, `js_planetscale`, `js_d1`, `js_better_sqlite3`, `js_mssql`, `js_mariadb`, `js_pg_cockroachdb`).
  - Bun runtime runs: use `pnpm --filter @prisma/client test:functional:code:bun --adapter js_bun_postgres <pattern>`; Bun-only adapter flavor is skipped unless `TEST_RUNTIME=bun`.
...
- **Creating new packages**:
  ...
  - **Important**: If the package has tests, add it to `.github/workflows/test-template.yml`. Utility packages typically go in:
    ...
    - `driver-adapter-unit-tests` job: includes `adapter-libsql`, `adapter-mariadb`, `adapter-d1`, `adapter-pg`, `adapter-bun-postgres`, `adapter-planetscale`, `adapter-mssql`, `adapter-neon`.
    - Bun functional CI coverage is Linux-only in `client-driveradapters-bun` and runs `js_bun_postgres` with Bun runtime for Prisma Client functional code tests.
