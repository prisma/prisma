...
- **Polymorphic relations** (issue #1644):
  ...
  - Rust commits in prisma-engines:
    - `bf336f64f28` feat: Add initial polymorphic relation support in query compiler
    - `b771993a8eb` fix: handle polymorphic fields in relation validation and query structure
    - `eda66122bbf` feat(psl): add @polymorphic attribute and DMMF support
    - `7e2cdbb458e` feat(psl): add polymorphic relation type parsing (Post | Comment)
  - Remaining work:
    - TypeScript client generators need to generate union types and `{ on: 'Post' }` parameter syntax
    - Query runtime needs discriminator-based filtering
    - Schema builder needs to generate polymorphic-specific input types (union of `{ on: Model, create/connect }` options)
