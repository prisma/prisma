...
- **Codebase helpers to know**:
  ...
  - Query-plan SQL `templateSql.fragments` may serialize scalar parameter placeholders as `null`; legacy `{ type: 'parameter' }` objects are still accepted. SQL string chunks are raw strings, while tuple and tuple-list parameter fragments remain object-shaped.
