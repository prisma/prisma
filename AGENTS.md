...
- **Client architecture (Prisma 7)**:
  ...
  - The query compiler wraps some single-statement plans (`updateMany`, `createMany`) in a `transaction` plan node. The interpreter does not start a transaction eagerly when the subtree contains at most one statement node (see `getMaxStatementNodeCount` in `interpreter/statement-count.ts`); it starts one lazily only if chunking splits that statement into multiple SQL queries. This keeps these operations working on driver adapters without transaction support (e.g. `PrismaNeonHttp`), see prisma/prisma#29748.
