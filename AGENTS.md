...
- **Client architecture (Prisma 7)**:
  ...
  - `client-engine-cache-timing.ts` also has render-only and data-map-only phase rows for cached plans. Current rows show `renderQuery()` for the first DB query and scalar data mapping are small relative to nested cached-plan execution; look at nested interpreter/join traversal and executor async overhead before more render-only tuning.
