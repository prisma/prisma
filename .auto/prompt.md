...
- JSON-only coverage output showed no measurable phase-tail saving, so the text reporter was restored. Deferring Cloudflare Postgres startup did not improve coverage, so the original workflow order was restored. The intended product diff is limited to `vitest.config.ts` and language-server harness tests.
