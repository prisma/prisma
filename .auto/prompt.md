...
- Three successful two-shard runs completed with critical paths of 592s, 587s, and 599s and maximum test steps of 534s, 533s, and 537s, always with the same 190+189 files and 1,099+1,030 tests. The 592.7s critical-path mean is 57.4% below the 1,392s historical median; the 534.7s maximum-step mean is 59.8% below the 1,331.5s historical median.
- Three-way Vitest sharding reduced the retained 531s two-shard critical path to 467s (12.1%) and the maximum test step from 475s to 392s, preserving all 379 files / 2,129 tests across 127+126+126 files. Aggregate integration runner time increased from 1,012s to 1,259s (24.4%), an explicit compute-cost tradeoff for the shorter gate.
- Four-way Vitest sharding produced critical paths of 324s and 305s (314.5s mean), down 31.9% from the retained 462s three-shard result, with maximum test steps of 273s and 237s. Both runs preserved all 379 files / 2,129 tests across 95+95+95+94 files and each used exactly 1,172s of aggregate integration runner time, versus 1,275s for the preceding three-shard run. Five-way sharding crossed the practical limit: its test step improved only to 235s, while fixed runner overhead pushed the critical path to 335s and aggregate runner time to 1,301s (+11.0%). Keep four shards and do not increase the count further.
- Inert-diff detection previously used a full-history checkout even though its action already fetches exact base/head SHAs when absent; the job varied from 59s to 308s and once delayed integration by more than four minutes. A shallow checkout completed exact-SHA detection in 10s on GitHub Actions, and the full workflow passed.
- The integration job provisioned a Postgres service even though no test under `test/integration` referenced a static Postgres URL or port; database tests provision explicit PGlite/Mongo connections. Removing it eliminated 22–28s of container initialization per shard. The first no-service run was masked by checkout/test variance at 323s, while the confirmation improved the retained 305s critical path to 295s and lowered aggregate runner time from 1,172s to 1,106s with full inventory.
...
- The SQL ORM collection helper also started PGlite per test across 32 files. Reusing one database per file while retaining the existing per-test drop/recreate schema path cut four representative files from 29.85s to 10.99s; all 41 SQL ORM files / 315 tests pass. The first hosted run was noisy at 633s, but the confirmation run improved the full CI critical path from the valid 599s baseline to 531s (11.4%) with all 379 files / 2,129 tests.
- A resettable per-file PGlite server for two ref/migration suites cut their focused runtime from 40.97s to 24.88s and hosted file timings from 31s/40s to 13s/30s. The reset must preserve `_prisma_dev_*` infrastructure schemas. Under four-way sharding those files are not on the critical shard, and the full CI path regressed from 305s to 320s, so the change was discarded.
- Opt-in journey config caching keyed by authored project files improved three focused files from 65.13s to 52.84s locally, but their hosted timings were unchanged or slower and the full critical path was 308s versus the retained 305s best. Hashing source files and retaining evaluated config added complexity without a hosted gain, so it was discarded.
- Caching every storage-hash database used by a Postgres port file cut two alternating-contract relation matrices from 69.80s to 20–25s locally and substantially improved their hosted per-file timings. Under two-way sharding, full CI critical paths of 548s and 569s both missed the then-retained 531s best, so the change was initially discarded. Three-way sharding later placed both matrices on the critical shard; reapplying the cache cut them from 103s/79s to 34s/40s, reduced the maximum test step from 392s to 369s, and improved the critical path from 467s to 462s with full inventory.
