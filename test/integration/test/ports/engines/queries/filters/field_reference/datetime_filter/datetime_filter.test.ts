...
function withMixedDateTimeFieldReference(
  fn: Parameters<typeof withPostgresPort<MixedContract>>[1],
) {
...
function withListDateTimeFieldReference(fn: Parameters<typeof withPostgresPort<ListContract>>[1]) {
...
function withDateTimeFieldReference(fn: Parameters<typeof withPostgresPort<Contract>>[1]) {
...
describe('ports/engines/queries/filters/field-reference/datetime-filter', () => {
  it(
    'basic_where',
    () =>
      withDateTimeFieldReference(async ({ db }) => {
        ...
        expect(await ids(BinaryExpr.eq(column('dt'), column('dt')))).toEqual([
          { id: 1 },
          { id: 2 },
        ]);
        expect(await ids(BinaryExpr.eq(column('dt'), column('dt2')))).toEqual([{ id: 1 }]);
        expect(await ids(new NotExpr(BinaryExpr.eq(column('dt'), column('dt2'))))).toEqual([
          { id: 2 },
        ]);
      }),
  ...
  it(
    ...
    () =>
      withDateTimeFieldReference(async ({ db }) => {
        ...
        expect(await ids(BinaryExpr.gt(column('dt'), column('dt')))).toEqual([]);
        expect(await ids(BinaryExpr.gt(column('dt2'), column('dt')))).toEqual([{ id: 2 }]);
        expect(await ids(new NotExpr(BinaryExpr.gt(column('dt'), column('dt'))))).toEqual([
          { id: 1 },
          { id: 2 },
        ]);
        expect(await ids(new NotExpr(BinaryExpr.gt(column('dt'), column('dt2'))))).toEqual([
          { id: 1 },
          { id: 2 },
        ]);

        expect(await ids(BinaryExpr.gte(column('dt'), column('dt')))).toEqual([
          { id: 1 },
          { id: 2 },
        ]);
        expect(await ids(BinaryExpr.gte(column('dt2'), column('dt')))).toEqual([
          { id: 1 },
          { id: 2 },
        ]);
        expect(await ids(new NotExpr(BinaryExpr.gte(column('dt'), column('dt'))))).toEqual([]);
        expect(await ids(new NotExpr(BinaryExpr.gte(column('dt'), column('dt2'))))).toEqual([
          { id: 2 },
        ]);

        expect(await ids(BinaryExpr.lt(column('dt'), column('dt')))).toEqual([]);
        expect(await ids(BinaryExpr.lt(column('dt'), column('dt2')))).toEqual([{ id: 2 }]);
        expect(await ids(new NotExpr(BinaryExpr.lt(column('dt'), column('dt'))))).toEqual([
          { id: 1 },
          { id: 2 },
        ]);
        expect(await ids(new NotExpr(BinaryExpr.lt(column('dt2'), column('dt'))))).toEqual([
          { id: 1 },
          { id: 2 },
        ]);

        expect(await ids(BinaryExpr.lte(column('dt'), column('dt')))).toEqual([
          { id: 1 },
          { id: 2 },
        ]);
        expect(await ids(BinaryExpr.lte(column('dt'), column('dt2')))).toEqual([
          { id: 1 },
          { id: 2 },
        ]);
        expect(await ids(new NotExpr(BinaryExpr.lte(column('dt'), column('dt'))))).toEqual([]);
        expect(await ids(new NotExpr(BinaryExpr.lte(column('dt2'), column('dt'))))).toEqual([
          { id: 2 },
        ]);
      }),
    timeouts.spinUpPpgDev,
  ...
  it(
    'inclusion_filter',
    () =>
      withMixedDateTimeFieldReference(async ({ db }) => {
        const ids = (filter: AnyExpression) =>
          db.public.TestModel.where(filter)
            .orderBy((row) => row.id.asc())
            .select('id')
  ...
  it(
    'scalar_list_filters',
    () =>
      withListDateTimeFieldReference(async ({ db }) => {
        const ids = (filter: AnyExpression) =>
          db.public.TestModel.where(filter)
            .orderBy((row) => row.id.asc())
            .select('id')
            .all();
        const scalar = column('dt');
        const list = column('dt_list');
        const otherList = column('dt_list2');
