...
describe('ports/engines/queries/filters/composite/some', () => {
  it(
    'basic',
    () =>
      withCompositeSome(basicDocuments, async ({ db }) => {
        const result = await queryIds(
          db,
          MongoFieldFilter.of('top_a', '$elemMatch', { a_2: { $lt: 0 } }),
        );
        expect(result).toEqual([{ _id: 3 }, { _id: 4 }]);
      }),
  ...
  it(
    ...
    () =>
      withCompositeSome(basicDocuments, async ({ db }) => {
        const some = await queryIds(db, anyTopA);
        expect(some).toEqual([{ _id: 1 }, { _id: 2 }, { _id: 3 }, { _id: 4 }, { _id: 5 }]);

        const notSome = await queryIds(
          db,
          MongoAndExpr.of([MongoExistsExpr.exists('top_a'), new MongoNotExpr(anyTopA)]),
        );
        expect(notSome).toEqual([{ _id: 6 }, { _id: 7 }]);
      }),
    timeouts.spinUpMongoMemoryServer,
  );
  ...
  it(
    'nested_some',
    () =>
      withCompositeSome(nestedDocuments, async ({ db }) => {
        const result = await queryIds(
          db,
          MongoFieldFilter.of('top_a', '$elemMatch', {
            a_to_many_bs: { $elemMatch: { b_field: { $lt: 0 } } },
          }),
        );
        expect(result).toEqual([{ _id: 2 }, { _id: 3 }]);
      }),
    timeouts.spinUpMongoMemoryServer,
  );
});
