// ⚠️  GENERATED FILE - DO NOT EDIT
// This file is automatically generated by 'prisma-next contract emit'.
// To regenerate, run: prisma-next contract emit
import type { QueryOperationTypes as PgAdapterQueryOps } from '@internal/adapter-postgres/operation-types';
import type {
  Bit,
  Char,
  CodecTypes as PgTypes,
  Interval,
  JsonValue,
  Numeric,
  Time,
  Timestamp,
  Timestamptz,
  Timetz,
  VarBit,
  Varchar,
} from '@internal/target-postgres/codec-types';

import type { ContractWithTypeMaps, TypeMaps as TypeMapsType } from '@internal/sql-contract/types';
import type {
  Contract as ContractType,
  ExecutionHashBase,
  NamespaceId,
  ProfileHashBase,
  StorageHashBase,
} from '@internal/contract/types';

export type StorageHash =
  StorageHashBase<'b24705cefe51680d6901a854971dc9ce26e4522065f1777c28426c53cecf0d5a'>;
export type ExecutionHash = ExecutionHashBase<string>;
export type ProfileHash =
  ProfileHashBase<'3916f444a8a17ad749191acf9e08dad97d1a327b88c2f1d45d12f240296aa8b2'>;

export type CodecTypes = PgTypes;
export type LaneCodecTypes = CodecTypes;
export type QueryOperationTypes = PgAdapterQueryOps<CodecTypes>;
export type AggregateTypes = {
  readonly avg: {
    readonly byCodec: {
      readonly 'pg/float@1': { readonly output: 'pg/float8@1'; readonly nullable: true };
      readonly 'pg/float4@1': { readonly output: 'pg/float8@1'; readonly nullable: true };
      readonly 'pg/float8@1': { readonly output: 'pg/float8@1'; readonly nullable: true };
      readonly 'pg/int@1': { readonly output: 'pg/numeric@1'; readonly nullable: true };
      readonly 'pg/int2@1': { readonly output: 'pg/numeric@1'; readonly nullable: true };
      readonly 'pg/int4@1': { readonly output: 'pg/numeric@1'; readonly nullable: true };
      readonly 'pg/int8@1': { readonly output: 'pg/numeric@1'; readonly nullable: true };
      readonly 'pg/interval@1': { readonly output: 'pg/interval@1'; readonly nullable: true };
      readonly 'pg/numeric@1': { readonly output: 'pg/numeric@1'; readonly nullable: true };
      readonly 'pg/time@1': { readonly output: 'pg/interval@1'; readonly nullable: true };
      readonly 'sql/float@1': { readonly output: 'pg/float8@1'; readonly nullable: true };
      readonly 'sql/int@1': { readonly output: 'pg/numeric@1'; readonly nullable: true };
    };
  };
  readonly count: {
    readonly byCodec: {};
    readonly withoutInput: { readonly output: 'pg/int8@1'; readonly nullable: false };
    readonly anyInput: { readonly output: 'pg/int8@1'; readonly nullable: false };
  };
  readonly max: {
    readonly byCodec: {
      readonly 'pg/char@1': { readonly output: 'pg/char@1'; readonly nullable: true };
      readonly 'pg/date@1': { readonly output: 'pg/date@1'; readonly nullable: true };
      readonly 'pg/enum@1': { readonly output: 'pg/enum@1'; readonly nullable: true };
      readonly 'pg/float@1': { readonly output: 'pg/float@1'; readonly nullable: true };
      readonly 'pg/float4@1': { readonly output: 'pg/float4@1'; readonly nullable: true };
      readonly 'pg/float8@1': { readonly output: 'pg/float8@1'; readonly nullable: true };
      readonly 'pg/inet@1': { readonly output: 'pg/inet@1'; readonly nullable: true };
      readonly 'pg/int@1': { readonly output: 'pg/int@1'; readonly nullable: true };
      readonly 'pg/int2@1': { readonly output: 'pg/int2@1'; readonly nullable: true };
      readonly 'pg/int4@1': { readonly output: 'pg/int4@1'; readonly nullable: true };
      readonly 'pg/int8@1': { readonly output: 'pg/int8@1'; readonly nullable: true };
      readonly 'pg/interval@1': { readonly output: 'pg/interval@1'; readonly nullable: true };
      readonly 'pg/numeric@1': { readonly output: 'pg/numeric@1'; readonly nullable: true };
      readonly 'pg/text-array@1': { readonly output: 'pg/text-array@1'; readonly nullable: true };
      readonly 'pg/text@1': { readonly output: 'pg/text@1'; readonly nullable: true };
      readonly 'pg/time@1': { readonly output: 'pg/time@1'; readonly nullable: true };
      readonly 'pg/timestamp@1': { readonly output: 'pg/timestamp@1'; readonly nullable: true };
      readonly 'pg/timestamptz@1': { readonly output: 'pg/timestamptz@1'; readonly nullable: true };
      readonly 'pg/timetz@1': { readonly output: 'pg/timetz@1'; readonly nullable: true };
      readonly 'pg/varchar@1': { readonly output: 'pg/text@1'; readonly nullable: true };
      readonly 'sql/char@1': { readonly output: 'sql/char@1'; readonly nullable: true };
      readonly 'sql/float@1': { readonly output: 'sql/float@1'; readonly nullable: true };
      readonly 'sql/int@1': { readonly output: 'sql/int@1'; readonly nullable: true };
      readonly 'sql/text@1': { readonly output: 'sql/text@1'; readonly nullable: true };
      readonly 'sql/timestamp@1': { readonly output: 'sql/timestamp@1'; readonly nullable: true };
      readonly 'sql/varchar@1': { readonly output: 'pg/text@1'; readonly nullable: true };
    };
  };
  readonly min: {
    readonly byCodec: {
      readonly 'pg/char@1': { readonly output: 'pg/char@1'; readonly nullable: true };
      readonly 'pg/date@1': { readonly output: 'pg/date@1'; readonly nullable: true };
      readonly 'pg/enum@1': { readonly output: 'pg/enum@1'; readonly nullable: true };
      readonly 'pg/float@1': { readonly output: 'pg/float@1'; readonly nullable: true };
      readonly 'pg/float4@1': { readonly output: 'pg/float4@1'; readonly nullable: true };
      readonly 'pg/float8@1': { readonly output: 'pg/float8@1'; readonly nullable: true };
      readonly 'pg/inet@1': { readonly output: 'pg/inet@1'; readonly nullable: true };
      readonly 'pg/int@1': { readonly output: 'pg/int@1'; readonly nullable: true };
      readonly 'pg/int2@1': { readonly output: 'pg/int2@1'; readonly nullable: true };
      readonly 'pg/int4@1': { readonly output: 'pg/int4@1'; readonly nullable: true };
      readonly 'pg/int8@1': { readonly output: 'pg/int8@1'; readonly nullable: true };
      readonly 'pg/interval@1': { readonly output: 'pg/interval@1'; readonly nullable: true };
      readonly 'pg/numeric@1': { readonly output: 'pg/numeric@1'; readonly nullable: true };
      readonly 'pg/text-array@1': { readonly output: 'pg/text-array@1'; readonly nullable: true };
      readonly 'pg/text@1': { readonly output: 'pg/text@1'; readonly nullable: true };
      readonly 'pg/time@1': { readonly output: 'pg/time@1'; readonly nullable: true };
      readonly 'pg/timestamp@1': { readonly output: 'pg/timestamp@1'; readonly nullable: true };
      readonly 'pg/timestamptz@1': { readonly output: 'pg/timestamptz@1'; readonly nullable: true };
      readonly 'pg/timetz@1': { readonly output: 'pg/timetz@1'; readonly nullable: true };
      readonly 'pg/varchar@1': { readonly output: 'pg/text@1'; readonly nullable: true };
      readonly 'sql/char@1': { readonly output: 'sql/char@1'; readonly nullable: true };
      readonly 'sql/float@1': { readonly output: 'sql/float@1'; readonly nullable: true };
      readonly 'sql/int@1': { readonly output: 'sql/int@1'; readonly nullable: true };
      readonly 'sql/text@1': { readonly output: 'sql/text@1'; readonly nullable: true };
      readonly 'sql/timestamp@1': { readonly output: 'sql/timestamp@1'; readonly nullable: true };
      readonly 'sql/varchar@1': { readonly output: 'pg/text@1'; readonly nullable: true };
    };
  };
  readonly sum: {
    readonly byCodec: {
      readonly 'pg/float@1': { readonly output: 'pg/float8@1'; readonly nullable: true };
      readonly 'pg/float4@1': { readonly output: 'pg/float4@1'; readonly nullable: true };
      readonly 'pg/float8@1': { readonly output: 'pg/float8@1'; readonly nullable: true };
      readonly 'pg/int@1': { readonly output: 'pg/int8@1'; readonly nullable: true };
      readonly 'pg/int2@1': { readonly output: 'pg/int8@1'; readonly nullable: true };
      readonly 'pg/int4@1': { readonly output: 'pg/int8@1'; readonly nullable: true };
      readonly 'pg/int8@1': { readonly output: 'pg/numeric@1'; readonly nullable: true };
      readonly 'pg/interval@1': { readonly output: 'pg/interval@1'; readonly nullable: true };
      readonly 'pg/numeric@1': { readonly output: 'pg/numeric@1'; readonly nullable: true };
      readonly 'pg/time@1': { readonly output: 'pg/interval@1'; readonly nullable: true };
      readonly 'sql/float@1': { readonly output: 'pg/float8@1'; readonly nullable: true };
      readonly 'sql/int@1': { readonly output: 'pg/int8@1'; readonly nullable: true };
    };
  };
};
type DefaultLiteralValue<CodecId extends string, Encoded> = CodecId extends keyof CodecTypes
  ? Encoded extends CodecTypes[CodecId]['json']
    ? Encoded
    : CodecTypes[CodecId]['json']
  : Encoded;

export type FieldOutputTypes = {
  readonly public: {
    readonly Bottom: {
      readonly bottomId: CodecTypes['pg/int4@1']['output'];
      readonly relatedMiddleId: CodecTypes['pg/int4@1']['output'] | null;
    };
    readonly Middle: { readonly middleId: CodecTypes['pg/int4@1']['output'] };
    readonly Top: {
      readonly topId: CodecTypes['pg/int4@1']['output'];
      readonly relatedMiddleId: CodecTypes['pg/int4@1']['output'] | null;
    };
    readonly TopBottom: {
      readonly topId: CodecTypes['pg/int4@1']['output'];
      readonly bottomId: CodecTypes['pg/int4@1']['output'];
    };
  };
};
export type FieldInputTypes = {
  readonly public: {
    readonly Bottom: {
      readonly bottomId: CodecTypes['pg/int4@1']['input'];
      readonly relatedMiddleId: CodecTypes['pg/int4@1']['input'] | null;
    };
    readonly Middle: { readonly middleId: CodecTypes['pg/int4@1']['input'] };
    readonly Top: {
      readonly topId: CodecTypes['pg/int4@1']['input'];
      readonly relatedMiddleId: CodecTypes['pg/int4@1']['input'] | null;
    };
    readonly TopBottom: {
      readonly topId: CodecTypes['pg/int4@1']['input'];
      readonly bottomId: CodecTypes['pg/int4@1']['input'];
    };
  };
};
export type StorageColumnTypes = {
  readonly public: {
    readonly bottom: {
      readonly bottomId: CodecTypes['pg/int4@1']['output'];
      readonly relatedMiddleId: CodecTypes['pg/int4@1']['output'] | null;
    };
    readonly middle: { readonly middleId: CodecTypes['pg/int4@1']['output'] };
    readonly top: {
      readonly relatedMiddleId: CodecTypes['pg/int4@1']['output'] | null;
      readonly topId: CodecTypes['pg/int4@1']['output'];
    };
    readonly topBottom: {
      readonly bottomId: CodecTypes['pg/int4@1']['output'];
      readonly topId: CodecTypes['pg/int4@1']['output'];
    };
  };
};
export type StorageColumnInputTypes = {
  readonly public: {
    readonly bottom: {
      readonly bottomId: CodecTypes['pg/int4@1']['input'];
      readonly relatedMiddleId: CodecTypes['pg/int4@1']['input'] | null;
    };
    readonly middle: { readonly middleId: CodecTypes['pg/int4@1']['input'] };
    readonly top: {
      readonly relatedMiddleId: CodecTypes['pg/int4@1']['input'] | null;
      readonly topId: CodecTypes['pg/int4@1']['input'];
    };
    readonly topBottom: {
      readonly bottomId: CodecTypes['pg/int4@1']['input'];
      readonly topId: CodecTypes['pg/int4@1']['input'];
    };
  };
};
export type TypeMaps = TypeMapsType<
  CodecTypes,
  QueryOperationTypes,
  FieldOutputTypes,
  FieldInputTypes,
  StorageColumnTypes,
  StorageColumnInputTypes,
  AggregateTypes
>;

type ContractBase = Omit<
  ContractType<{
    readonly namespaces: {
      readonly public: {
        readonly id: 'public';
        readonly kind: 'postgres-schema';
        readonly entries: {
          readonly table: {
            readonly bottom: {
              columns: {
                readonly bottomId: {
                  readonly nativeType: 'int4';
                  readonly codecId: 'pg/int4@1';
                  readonly nullable: false;
                };
                readonly relatedMiddleId: {
                  readonly nativeType: 'int4';
                  readonly codecId: 'pg/int4@1';
                  readonly nullable: true;
                };
              };
              primaryKey: { readonly columns: readonly ['bottomId'] };
              uniques: readonly [];
              indexes: readonly [
                {
                  readonly name: 'bottom_relatedMiddleId_idx_ec5ed7cd';
                  readonly prefix: 'bottom_relatedMiddleId_idx';
                  readonly columns: readonly ['relatedMiddleId'];
                  readonly unique: false;
                },
              ];
              foreignKeys: readonly [
                {
                  readonly source: {
                    readonly namespaceId: 'public' & NamespaceId;
                    readonly tableName: 'bottom';
                    readonly columns: readonly ['relatedMiddleId'];
                  };
                  readonly target: {
                    readonly namespaceId: 'public' & NamespaceId;
                    readonly tableName: 'middle';
                    readonly columns: readonly ['middleId'];
                  };
                },
              ];
            };
            readonly middle: {
              columns: {
                readonly middleId: {
                  readonly nativeType: 'int4';
                  readonly codecId: 'pg/int4@1';
                  readonly nullable: false;
                };
              };
              primaryKey: { readonly columns: readonly ['middleId'] };
              uniques: readonly [];
              indexes: readonly [];
              foreignKeys: readonly [];
            };
            readonly top: {
              columns: {
                readonly topId: {
                  readonly nativeType: 'int4';
                  readonly codecId: 'pg/int4@1';
                  readonly nullable: false;
                };
                readonly relatedMiddleId: {
                  readonly nativeType: 'int4';
                  readonly codecId: 'pg/int4@1';
                  readonly nullable: true;
                };
              };
              primaryKey: { readonly columns: readonly ['topId'] };
              uniques: readonly [];
              indexes: readonly [
                {
                  readonly name: 'top_relatedMiddleId_idx_ec5ed7cd';
                  readonly prefix: 'top_relatedMiddleId_idx';
                  readonly columns: readonly ['relatedMiddleId'];
                  readonly unique: false;
                },
              ];
              foreignKeys: readonly [
                {
                  readonly source: {
                    readonly namespaceId: 'public' & NamespaceId;
                    readonly tableName: 'top';
                    readonly columns: readonly ['relatedMiddleId'];
                  };
                  readonly target: {
                    readonly namespaceId: 'public' & NamespaceId;
                    readonly tableName: 'middle';
                    readonly columns: readonly ['middleId'];
                  };
                },
              ];
            };
            readonly topBottom: {
              columns: {
                readonly topId: {
                  readonly nativeType: 'int4';
                  readonly codecId: 'pg/int4@1';
                  readonly nullable: false;
                };
                readonly bottomId: {
                  readonly nativeType: 'int4';
                  readonly codecId: 'pg/int4@1';
                  readonly nullable: false;
                };
              };
              primaryKey: { readonly columns: readonly ['topId', 'bottomId'] };
              uniques: readonly [];
              indexes: readonly [
                {
                  readonly name: 'topBottom_topId_idx_706dce97';
                  readonly prefix: 'topBottom_topId_idx';
                  readonly columns: readonly ['topId'];
                  readonly unique: false;
                },
                {
                  readonly name: 'topBottom_bottomId_idx_ea943a00';
                  readonly prefix: 'topBottom_bottomId_idx';
                  readonly columns: readonly ['bottomId'];
                  readonly unique: false;
                },
              ];
              foreignKeys: readonly [
                {
                  readonly source: {
                    readonly namespaceId: 'public' & NamespaceId;
                    readonly tableName: 'topBottom';
                    readonly columns: readonly ['topId'];
                  };
                  readonly target: {
                    readonly namespaceId: 'public' & NamespaceId;
                    readonly tableName: 'top';
                    readonly columns: readonly ['topId'];
                  };
                },
                {
                  readonly source: {
                    readonly namespaceId: 'public' & NamespaceId;
                    readonly tableName: 'topBottom';
                    readonly columns: readonly ['bottomId'];
                  };
                  readonly target: {
                    readonly namespaceId: 'public' & NamespaceId;
                    readonly tableName: 'bottom';
                    readonly columns: readonly ['bottomId'];
                  };
                },
              ];
            };
          };
        };
      };
    };
    readonly storageHash: StorageHash;
  }>,
  'roots' | 'domain'
> & {
  readonly target: 'postgres';
  readonly targetFamily: 'sql';
  readonly roots: {
    readonly top: { readonly namespace: 'public' & NamespaceId; readonly model: 'Top' };
    readonly middle: { readonly namespace: 'public' & NamespaceId; readonly model: 'Middle' };
    readonly bottom: { readonly namespace: 'public' & NamespaceId; readonly model: 'Bottom' };
    readonly topBottom: { readonly namespace: 'public' & NamespaceId; readonly model: 'TopBottom' };
  };
  readonly domain: {
    readonly namespaces: {
      readonly public: {
        readonly models: {
          readonly Bottom: {
            readonly fields: {
              readonly bottomId: {
                readonly nullable: false;
                readonly type: { readonly kind: 'scalar'; readonly codecId: 'pg/int4@1' };
              };
              readonly relatedMiddleId: {
                readonly nullable: true;
                readonly type: { readonly kind: 'scalar'; readonly codecId: 'pg/int4@1' };
              };
            };
            readonly relations: {
              readonly middle: {
                readonly to: {
                  readonly namespace: 'public' & NamespaceId;
                  readonly model: 'Middle';
                };
                readonly cardinality: 'N:1';
                readonly on: {
                  readonly localFields: readonly ['relatedMiddleId'];
                  readonly targetFields: readonly ['middleId'];
                };
              };
              readonly tops: {
                readonly to: { readonly namespace: 'public' & NamespaceId; readonly model: 'Top' };
                readonly cardinality: 'N:M';
                readonly on: {
                  readonly localFields: readonly ['bottomId'];
                  readonly targetFields: readonly ['bottomId'];
                };
                readonly through: {
                  readonly table: 'topBottom';
                  readonly namespaceId: 'public';
                  readonly parentColumns: readonly ['bottomId'];
                  readonly childColumns: readonly ['topId'];
                  readonly targetColumns: readonly ['topId'];
                };
              };
            };
            readonly storage: {
              readonly table: 'bottom';
              readonly namespaceId: 'public';
              readonly fields: {
                readonly bottomId: { readonly column: 'bottomId' };
                readonly relatedMiddleId: { readonly column: 'relatedMiddleId' };
              };
            };
          };
          readonly Middle: {
            readonly fields: {
              readonly middleId: {
                readonly nullable: false;
                readonly type: { readonly kind: 'scalar'; readonly codecId: 'pg/int4@1' };
              };
            };
            readonly relations: {
              readonly bottoms: {
                readonly to: {
                  readonly namespace: 'public' & NamespaceId;
                  readonly model: 'Bottom';
                };
                readonly cardinality: '1:N';
                readonly on: {
                  readonly localFields: readonly ['middleId'];
                  readonly targetFields: readonly ['relatedMiddleId'];
                };
              };
              readonly tops: {
                readonly to: { readonly namespace: 'public' & NamespaceId; readonly model: 'Top' };
                readonly cardinality: '1:N';
                readonly on: {
                  readonly localFields: readonly ['middleId'];
                  readonly targetFields: readonly ['relatedMiddleId'];
                };
              };
            };
            readonly storage: {
              readonly table: 'middle';
              readonly namespaceId: 'public';
              readonly fields: { readonly middleId: { readonly column: 'middleId' } };
            };
          };
          readonly Top: {
            readonly fields: {
              readonly topId: {
                readonly nullable: false;
                readonly type: { readonly kind: 'scalar'; readonly codecId: 'pg/int4@1' };
              };
              readonly relatedMiddleId: {
                readonly nullable: true;
                readonly type: { readonly kind: 'scalar'; readonly codecId: 'pg/int4@1' };
              };
            };
            readonly relations: {
              readonly bottoms: {
                readonly to: {
                  readonly namespace: 'public' & NamespaceId;
                  readonly model: 'Bottom';
                };
                readonly cardinality: 'N:M';
                readonly on: {
                  readonly localFields: readonly ['topId'];
                  readonly targetFields: readonly ['topId'];
                };
                readonly through: {
                  readonly table: 'topBottom';
                  readonly namespaceId: 'public';
                  readonly parentColumns: readonly ['topId'];
                  readonly childColumns: readonly ['bottomId'];
                  readonly targetColumns: readonly ['bottomId'];
                };
              };
              readonly middle: {
                readonly to: {
                  readonly namespace: 'public' & NamespaceId;
                  readonly model: 'Middle';
                };
                readonly cardinality: 'N:1';
                readonly on: {
                  readonly localFields: readonly ['relatedMiddleId'];
                  readonly targetFields: readonly ['middleId'];
                };
              };
            };
            readonly storage: {
              readonly table: 'top';
              readonly namespaceId: 'public';
              readonly fields: {
                readonly topId: { readonly column: 'topId' };
                readonly relatedMiddleId: { readonly column: 'relatedMiddleId' };
              };
            };
          };
          readonly TopBottom: {
            readonly fields: {
              readonly topId: {
                readonly nullable: false;
                readonly type: { readonly kind: 'scalar'; readonly codecId: 'pg/int4@1' };
              };
              readonly bottomId: {
                readonly nullable: false;
                readonly type: { readonly kind: 'scalar'; readonly codecId: 'pg/int4@1' };
              };
            };
            readonly relations: {
              readonly bottom: {
                readonly to: {
                  readonly namespace: 'public' & NamespaceId;
                  readonly model: 'Bottom';
                };
                readonly cardinality: 'N:1';
                readonly on: {
                  readonly localFields: readonly ['bottomId'];
                  readonly targetFields: readonly ['bottomId'];
                };
              };
              readonly top: {
                readonly to: { readonly namespace: 'public' & NamespaceId; readonly model: 'Top' };
                readonly cardinality: 'N:1';
                readonly on: {
                  readonly localFields: readonly ['topId'];
                  readonly targetFields: readonly ['topId'];
                };
              };
            };
            readonly storage: {
              readonly table: 'topBottom';
              readonly namespaceId: 'public';
              readonly fields: {
                readonly topId: { readonly column: 'topId' };
                readonly bottomId: { readonly column: 'bottomId' };
              };
            };
          };
        };
      };
    };
  };
  readonly capabilities: {
    readonly postgres: {
      readonly distinctOn: true;
      readonly jsonAgg: true;
      readonly lateral: true;
      readonly limit: true;
      readonly orderBy: true;
      readonly returning: true;
    };
    readonly sql: {
      readonly defaultInInsert: true;
      readonly enums: true;
      readonly lateral: true;
      readonly returning: true;
      readonly scalarList: true;
    };
  };
  readonly extensions: {};
  readonly meta: {};

  readonly profileHash: ProfileHash;
};

export type Contract = ContractWithTypeMaps<ContractBase, TypeMaps>;

export type Namespaces = Contract['storage']['namespaces'];
