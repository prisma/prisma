...
- `query-engine/connector-test-kit-rs/query-engine-tests/tests/queries/data_types/native/postgres.rs` › `queries::data_types::native::postgres::native_string` — reads Char, VarChar, Text, Bit, VarBit, Uuid, and Inet native fields together — SQL PSL rejects Bit and VarBit field constructors, so the faithful schema cannot be authored
