...
- `query-engine/connector-test-kit-rs/query-engine-tests/tests/queries/data_types/enum_type.rs` › `queries::data_types::enum_type::read_one_invalid_mongo` — reading an invalid enum value manually inserted into Mongo yields the query-engine enum conversion error — the faithful source schema uses an Int Mongo `_id`, but prisma-next's Mongo PSL requires an ObjectId `_id` (`PSL_MONGO_ID_REQUIRED`); changing the id type would simplify the assigned schema
