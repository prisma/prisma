 ...
 * quote (https://github.com/prisma/orm/issues/30213).
 *
 * A double quote is legal inside a quoted Postgres identifier when it is
 * doubled, so `quoted"table` has to render as `"quoted""table"`. Table
 * qualification interpolated the name verbatim instead, producing
 * `"quoted"table"` — a syntax error in every statement the name reaches. This
 * test drives the whole path from public API: it migrates the contract onto a
 * real database (CREATE TABLE) and then writes and reads rows through the typed
...
import type { Contract } from '../sql-orm-client/fixtures/quoted-table-name/generated/contract';
import contractJson from '../sql-orm-client/fixtures/quoted-table-name/generated/contract.json' with {
  type: 'json',
};
...
describe('ORM round-trip over a quote-containing table name', { concurrent: false }, () => {
  ...
  it(
    'writes and reads a @@map("quoted\\"table") model through the typed ORM',
    async () => {
      ...
      await withClient(database.connectionString, async (client) => {
        ...
        // The physical name really is the quote-containing one — the round trip
        // above would pass just as well against an unmapped table.
        const physical = await client.query(`SELECT value FROM "quoted""table" WHERE id = 1`);
        expect(physical.rows).toEqual([{ value: 'written through the ORM' }]);
      });
    },
    timeouts.spinUpPpgDev,
  );
