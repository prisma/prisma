...
class D1HttpQueryable implements SqlQueryable {
  ...
  readonly driverName = `${packageName}-http`
...
export class PrismaD1HttpAdapter extends D1HttpQueryable implements SqlDriver {
  readonly tags = {
...
export class PrismaD1HttpDriverFactory implements SqlMigrationAwareDriverFactory {
  readonly provider = 'sqlite'
  readonly driverName = `${packageName}-http`
  ...
  async connectToShadowDb(): Promise<SqlDriver> {
