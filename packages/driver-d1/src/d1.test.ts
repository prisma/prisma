...
import { mockAdapter, SqlDriverFactory, SqlMigrationAwareDriverFactory } from '@prisma/driver-utils'
import { afterEach, describe, expect, expectTypeOf, test, vi } from 'vitest'

import { PrismaD1Http } from '.'
import { PrismaD1 } from './d1'
import { PrismaD1HttpDriverFactory } from './d1-http'
...
describe('D1 adapter instance creation', () => {
  ...
  test('create a migration-aware adapter with cloudflare env variables', async () => {
    ...
    const connect = vi.spyOn(PrismaD1HttpDriverFactory.prototype, 'connect').mockResolvedValue(adapter)
    const connectToShadowDb = vi
      .spyOn(PrismaD1HttpDriverFactory.prototype, 'connectToShadowDb')
    ...
    expectTypeOf(factory).toExtend<SqlMigrationAwareDriverFactory>()
  ...
  test('create a non-migration-aware adapter with a D1 database object', async () => {
    const connect = vi.spyOn(PrismaD1WorkerDriverFactory.prototype, 'connect').mockResolvedValue(mockAdapter('sqlite'))

    const d1 = {} as D1Database
    const factory = new PrismaD1(d1)
    await factory.connect()

    expectTypeOf(factory).toExtend<SqlDriverFactory>()
    expectTypeOf(factory).not.toExtend<SqlMigrationAwareDriverFactory>()
