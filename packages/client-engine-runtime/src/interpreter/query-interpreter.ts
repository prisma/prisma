...
export class QueryInterpreter {
  ...
  async run(queryPlan: QueryPlanNode, queryable: SqlQueryable): Promise<unknown> {
    ...
    const purified = purifyQueryPlan(queryPlan, (node) =>
      this.interpretNode(node, queryable, this.#placeholderValues, generators),
    )?.catch((e) => rethrowAsUserFacing(e))

    if (purified) {
      return this.interpretPureNode(await purified, this.#placeholderValues, generators).value
    }
  ...
  private async interpretNode(
    ...
    switch (node.type) {
      ...
      default:
        return this.interpretPureNode(node, scope, generators)
    }
  }

  private interpretPureNode(
    node: PureQueryPlanNode,
    scope: ScopeBindings,
    generators: GeneratorRegistrySnapshot,
  ): IntermediateValue {
    switch (node.type) {
...
export function purifyQueryPlan(
  node: QueryPlanNode,
  evalNode: (node: ImpureQueryPlanNode) => Promise<IntermediateValue>,
): Promise<PureQueryPlanNode> | undefined {
  ...
  if (uniqueAsyncNode) {
    return evalNode(uniqueAsyncNode).then((result) => {
      ...
      return node as PureQueryPlanNode
    })
  }
  return
}

function findUniqueUnconditionalImpureNode(node: QueryPlanNode): ImpureQueryPlanNode | null | undefined {
  switch (node.type) {
    case 'query':
    case 'execute':
      return node
    ...
    case 'sum':
    case 'concat': {
      let found: ImpureQueryPlanNode | undefined = undefined
