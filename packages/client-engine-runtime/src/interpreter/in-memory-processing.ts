 ...
 */
export function getRecordKey(record: {}, fields: string[], mappers?: ((value: unknown) => unknown)[]): string {
  const array = fields.map((field, index) => (mappers?.[index] ? mappers[index](record[field]) : record[field]))
