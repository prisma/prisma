...
class LibSqlQueryable<ClientT extends StdClient | TransactionClient> implements SqlQueryable {
  ...
  constructor(
    ...
    protected readonly driverOptions?: PrismaLibSqlOptions,
  ) {}
  ...
  private async performIO(query: SqlQuery): Promise<LibSqlResultSet> {
    ...
    try {
      const result = await this.client.execute({
        sql: query.sql,
        args: query.args.map((arg, i) => mapArg(arg, query.argTypes[i], this.driverOptions)),
      })
      return result
    } catch (e) {
      this.onError(e)
...
class LibSqlTransaction extends LibSqlQueryable<TransactionClient> implements Transaction {
  ...
  constructor(
    ...
    driverOptions: PrismaLibSqlOptions | undefined,
    unlockParent: () => void,
  ) {
...
export class PrismaLibSqlAdapter extends LibSqlQueryable<StdClient> implements SqlDriver {
  constructor(client: StdClient, driverOptions?: PrismaLibSqlOptions) {
    super(client, driverOptions)
  }
  ...
  async startTransaction(isolationLevel?: IsolationLevel): Promise<Transaction> {
    ...
    try {
      ...
      return new LibSqlTransaction(tx, options, this.driverOptions, release)
    } catch (e) {
...
export abstract class PrismaLibSqlDriverFactoryBase implements SqlMigrationAwareDriverFactory {
  readonly provider = 'sqlite'
  readonly driverName = packageName
