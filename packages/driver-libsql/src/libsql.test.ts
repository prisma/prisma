...
import { ColumnTypeEnum, IsolationLevel, SqlMigrationAwareDriverFactory } from '@prisma/driver-utils'
import { describe, expect, test, vi } from 'vitest'

import { PrismaLibSqlDriverFactoryBase } from './libsql'
import { PrismaLibSqlDriverFactory } from './libsql-node'

describe.each([
  (factory: SqlMigrationAwareDriverFactory) => factory.connect(),
  (factory: SqlMigrationAwareDriverFactory) => factory.connectToShadowDb(),
])('behavior of the adapter with "%s"', (connect) => {
  test('executes and parses simple queries', async () => {
    const factory = new PrismaLibSqlDriverFactory({ url: ':memory:' })
...
describe.each([
  ...
  test('dispose closes the underlying connection', async () => {
    const factory = new PrismaLibSqlDriverFactoryMock({ url: ':memory:' })
    const conn = await connect(factory)
...
class PrismaLibSqlDriverFactoryMock extends PrismaLibSqlDriverFactoryBase {
  connection = {
    execute: vi.fn(),
    batch: vi.fn(),
    close: vi.fn(),
