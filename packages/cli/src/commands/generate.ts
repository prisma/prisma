...
import { TypeGenerator, EnhancedClientGeneratorWithTranslations } from '@refract/client'
...
export class GenerateCommand extends BaseCommand {
  async execute(options: GenerateOptions = {}): Promise<CommandResult> {
    ...
    try {
      ...
      const clientFile = await this.writeEnhancedClientFile(resolvedOutputDir, schemaAST)
...
`
   ...
   * Write enhanced client file with pre-compiled operations
   */
  private async writeEnhancedClientFile(outputDir: string, schemaAST: any): Promise<string> {
    const clientPath = path.join(outputDir, 'index.ts')
    
    try {
      console.log('🔧 Using enhanced client generation with pre-compiled operations')
      
      // Generate enhanced client with pre-compiled operations and field translations
      const enhancedGenerator = new EnhancedClientGeneratorWithTranslations(schemaAST, {
        includeTypes: true,
        includeJSDoc: true,
        esModules: true,
      })
      
      const clientContent = enhancedGenerator.generateEnhancedClient()
      console.log('✓ Enhanced client content generated, length:', clientContent.length)
      
      await fs.writeFile(clientPath, clientContent, 'utf-8')
      console.log('✓ Enhanced client written to:', clientPath)
      
      return clientPath
    } catch (error) {
      console.error('❌ Enhanced client generation failed, falling back to legacy:', error)
...
import { RefractClient, createRefractClientFromConfig } from '@refract/client'
...
export function createClientWithConfig(config: any) {
  return new RefractClient<DatabaseSchema>(config)
}
