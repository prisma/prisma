...
export function determineClientOutputPath(schemaDir: string): string {
  ...
  return relativeOutputPath.replace(path.sep, '/')
}
...
const tsconfigSchema = Schema.partial(
  Schema.Struct(
    {
      extends: Schema.String,
      compilerOptions: Schema.partial(
        Schema.Struct(
          {
            rootDir: Schema.String,
            rootDirs: Schema.Array(Schema.String),
...
const parseTsConfig = Schema.decodeUnknownSync(tsconfigSchema)
