    ...
    if (args['--check']) {
      for (const [filename, formattedSchema] of formattedDatamodel) {
        ...
        if (!originalSchemaTuple) {
          return new Error(`The schema ${underline(filename)} is not found in the schema list.`)
        }
        const [, originalSchema] = originalSchemaTuple
        if (originalSchema !== formattedSchema) {
          return new Error(`There are unformatted files. Run ${underline('prisma format')} to format them.`)
        }
      }
