  ...
  async parse(argv: string[], _config: PrismaConfigInternal): Promise<string | Error> {
    ...
    if (isPpgCommand) {
      ...
      if (!credentials) {
        ...
        await saveCredentials(await login({ utmMedium: 'command-prisma-init-db' }))
      }
      ...
      const client = await createAuthenticatedManagementApiClient({ utmMedium: 'command-prisma-init-db' })
