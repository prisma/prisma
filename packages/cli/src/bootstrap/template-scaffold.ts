...
const PRISMA_EXAMPLES_TARBALL_URL =
	"https://api.github.com/repos/prisma/prisma-examples/tarball/latest";
...
export async function downloadAndExtractTemplate(
	...
	const tarBuffer = await decompressGzip(
		...
		if (header.type === "0" || header.type === "") {
			...
			if (relPath?.startsWith(templatePrefix)) {
				const destPath = path.resolve(
					targetDir,
					relPath.slice(templatePrefix.length),
				);
				if (!destPath.startsWith(path.resolve(targetDir) + path.sep)) continue;
				fs.mkdirSync(path.dirname(destPath), { recursive: true });
				fs.writeFileSync(
					destPath,
					tarBuffer.subarray(offset, offset + header.size),
				);
				filesExtracted++;
			}
		}
	...
	if (filesExtracted === 0) {
		throw new Error(
			`Template "${templateName}" not found in prisma-examples repository`,
		);
	}
