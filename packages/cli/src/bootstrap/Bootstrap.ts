...
} from "../postgres/link/management-api";
import {
...
`);
	...
	private async run(
		...
		try {
			...
			const isEmptyProject =
				!initialState.hasSchemaFile && !initialState.hasPackageJson;
			...
			if (!initialState.hasSchemaFile) {
				if (isEmptyProject) {
					...
					console.log(
						`  A ${bold("package.json")} is required for Prisma to work.`,
					);
					console.log(
				...
				if (useTemplate) {
					const spinner = ora(
						`Downloading ${bold(useTemplate)} template...`,
					).start();
					...
					try {
						...
						spinner.fail(
							`Template download failed: ${sanitizeErrorMessage(msg)}`,
						);
						...
						await this.runInit(
							...
							telemetryCtx,
							config,
							await this.askAboutSampleModel(),
						);
					}
			...
			if (!force && isAlreadyLinked(baseDir)) {
				...
				try {
					const link = Link.new();
					const linkResult = await link.link(apiKey, databaseId, baseDir, {
						force,
					});
					...
					await emitStepCompleted(
						telemetryCtx,
						"link",
						performance.now() - linkStart,
					);

					console.log(
						`${green("✔")} Linked to database ${bold(linkResult.databaseId)}`,
					);
				} catch (err) {
					const msg = err instanceof Error ? err.message : String(err);
			...
			if (templateScaffolded) {
				...
				try {
					...
					const msg = err instanceof Error ? err.message : String(err);
					installSpinner.fail(
						`Dependency install failed: ${sanitizeErrorMessage(msg)}`,
					);
					await emitStepFailed(
						telemetryCtx,
						"install_deps",
			...
			const subprocessEnv: Record<string, string> = databaseUrl
			...
			if (!initialState.hasPrismaConfig && updatedState.hasPrismaConfig) {
				try {
					...
					console.log(
						`${yellow("warn")} Could not reload config — using initial config for migrate/seed`,
					);
				}
			}
			...
			if (needsDepsInstall) {
				...
				console.log(
					`  Install them as dev dependencies with your package manager, then re-run:`,
				);
				console.log(`  ${dim("$")} npx prisma@latest bootstrap`);

				return formatBootstrapOutput({
					databaseId:
						telemetryCtx.linkResult?.databaseId ?? databaseId ?? "unknown",
			...
			const useLocalBin =
				localPrismaBin !== null &&
				(initialState.hasSchemaFile || templateScaffolded);
			...
			if (updatedState.hasModels) {
				...
				const modelSummary =
					modelCount > 0
						? ` ${modelCount} model${modelCount === 1 ? "" : "s"} (${modelNames.join(", ")})`
						: " schema";
				...
				if (shouldMigrate) {
					...
					try {
						if (useLocalBin && localPrismaBin) {
							...
							const migrateResult = await migrateDev.parse(
								["--name", "init"],
								activeConfig,
								baseDir,
							);
							...
							if (migrateResult instanceof Error) {
								...
								console.log(
									`${yellow("warn")} Migration failed: ${sanitizeErrorMessage(migrateResult.message)}`,
								);
								await emitStepFailed(
									telemetryCtx,
									"migrate",
									sanitizeErrorMessage(migrateResult.message),
								);
							}
						}
						...
						if (steps.migrate !== "failed") {
							...
							await emitStepCompleted(
								telemetryCtx,
								"migrate",
								performance.now() - migrateStart,
							);
						}
					} catch (err) {
						const msg = err instanceof Error ? err.message : String(err);
						console.log(
							`${yellow("warn")} Migration failed: ${sanitizeErrorMessage(msg)}`,
						);
						steps.migrate = "failed";
						await emitStepFailed(
							telemetryCtx,
				...
				try {
					if (useLocalBin && localPrismaBin) {
						...
						if (generateResult instanceof Error) {
							...
							console.log(
								`${yellow("warn")} Generate failed: ${sanitizeErrorMessage(generateResult.message)}`,
							);
							await emitStepFailed(
								telemetryCtx,
								"generate",
								sanitizeErrorMessage(generateResult.message),
							);
						}
					}
					...
					const msg = err instanceof Error ? err.message : String(err);
					console.log(
						`${yellow("warn")} Generate failed: ${sanitizeErrorMessage(msg)}`,
					);
					steps.generate = "failed";
					await emitStepFailed(
						telemetryCtx,
					...
					try {
						if (useLocalBin && localPrismaBin) {
							...
							if (seedResult instanceof Error) {
								steps.seed = "failed";
								console.log(
									`${yellow("warn")} Seed failed: ${sanitizeErrorMessage(seedResult.message)}`,
								);
								await emitStepFailed(
									telemetryCtx,
									"seed",
									sanitizeErrorMessage(seedResult.message),
								);
							}
						}
						...
						const msg = err instanceof Error ? err.message : String(err);
						console.log(
							`${yellow("warn")} Seed failed: ${sanitizeErrorMessage(msg)}`,
						);
						steps.seed = "failed";
						await emitStepFailed(
							telemetryCtx,
	...
	private async runInit(
		...
		const label = withModel
			? "Setting up Prisma with a sample User model..."
			: "Setting up Prisma...";
		console.log(`\n${bold(label)}`);
		const stepStart = performance.now();
		...
		try {
			...
			if (initResult instanceof Error) {
				await emitStepFailed(
					telemetryCtx,
					"init",
					sanitizeErrorMessage(initResult.message),
				);
			...
			if (err instanceof Error && err.message.startsWith("Init failed:"))
				throw err;
			const msg = err instanceof Error ? err.message : String(err);
