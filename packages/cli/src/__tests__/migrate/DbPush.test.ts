...
import { DbPush } from '../../migrate/commands/DbPush'
import { setupMongo, SetupParams, tearDownMongo } from '../../migrate/utils/setupMongo'
...
describeIf(!process.env.TEST_SKIP_MONGODB)('push existing-db with mongodb', () => {
  ...
  const setupParams: SetupParams = {
    ...
    dirname: path.join(__dirname, '..', 'fixtures', 'existing-db-warnings-mongodb'),
  }
