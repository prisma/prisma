...
bench('client extensions(1)- 1 op', () => {
  ...
  xprisma.user.findFirst({ where: { id: 'initial_id' } })
}).types([4160, 'instantiations'])
...
bench('client extensions(1)- 5 ops', () => {
  ...
  xprisma.user.findFirst({ where: { id: 'initial_id_op1' } })
  xprisma.link.findMany({ where: { userId: 'some_user_id_op2' } })
  xprisma.user.create({
    data: { email: 'op3@example.com', name: 'Op 3 User' },
  })
  xprisma.link.update({
    where: { id: 'link_id_op4' },
    data: { shortUrl: 'updated-op4' },
  })
  xprisma.user.findUnique({ where: { email: 'op5@example.com' } })
}).types([4160, 'instantiations'])

bench('client extensions(2)', () => {
...
bench('client extensions(5)- 5 ops', () => {
  ...
  xprisma.user.findUnique({ where: { email: 'op5@example.com' } })
}).types([19045, 'instantiations'])

bench('client extensions(10)', () => {
  const xprisma = prisma
    .$extends({
      query: {
        user: {
    ...
    .$extends({
      query: {
        link: {
          create({ args, query }) {
            args.data.shortUrl = args.data.shortUrl + '-ext8'
            return query(args)
          },
        },
      },
      ...
      query: {
        user: {
          findUnique({ args, query }) {
            console.log('Final extension findUnique user')
            return query(args)
          },
        },
      },
