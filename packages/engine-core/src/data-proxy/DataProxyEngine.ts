...
import { timedRequest } from '../common/utils/util'
...
export class DataProxyEngine extends Engine {
  ...
  constructor(config: EngineConfig) {
    ...
    this.queryTimeout = config.queryTimeout ?? DEFAULT_REQUEST_TIMEOUT_MS
  ...
  request<T>(query: string, headers: Record<string, string>, attempt = 0) {
    ...
    return timedRequest(
      this.requestInternal<T>({ query, variables: {} }, headers, attempt),
      this.queryTimeout,
      this.clientVersion,
    )
  }
  ...
  async requestBatch<T>(queries: string[], headers: Record<string, string>, isTransaction = false, attempt = 0) {
    ...
    const { batchResult } = await timedRequest(
      this.requestInternal<T>(body, headers, attempt),
      this.queryTimeout,
      this.clientVersion,
    )
