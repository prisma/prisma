...
import { PrismaPgDriverFactory } from '../pg'

describe('PrismaPgDriverFactory', () => {
  it('should subscribe to pool error events', async () => {
    ...
    const factory = new PrismaPgDriverFactory(config)
  ...
  it('should call onPoolError when supplied', async () => {
    ...
    const factory = new PrismaPgDriverFactory(config, { onPoolError })
    const adapter = await factory.connect()
  ...
  it('should add and remove error event listener when using an external Pool', async () => {
    ...
    pool.on('error', () => {})
    const factory = new PrismaPgDriverFactory(pool)
    const adapter = await factory.connect()
    expect(adapter).toBeDefined()
    expect(adapter.driverName).toBeDefined()
