 ...
 * Generated at: 2025-10-30T15:28:13.204Z
 */
...
class UserOperations {
  ...
  private applyWhereConditions(query: any, where: Record<string, unknown>): any {
    ...
    for (const [field, value] of Object.entries(where)) {
      ...
      if (field === 'AND') {
        ...
        currentQuery = currentQuery.where((eb: any) => {
          let andQuery = eb
          for (const condition of conditions) {
            andQuery = this.applyWhereConditionsToBuilder(andQuery, condition as Record<string, unknown>)
          }
          return andQuery
        })
        continue
      ...
      if (field === 'OR') {
        ...
        currentQuery = currentQuery.where((eb: any) => {
          return eb.or(conditions.map((condition: any) =>
            (subEb: any) => this.applyWhereConditionsToBuilder(subEb, condition)
          ))
        })
        continue
      }
      ...
      if (field === 'NOT') {
        ...
        for (const condition of conditions) {
          currentQuery = currentQuery.where((eb: any) => {
            return eb.not(this.applyWhereConditionsToBuilder(eb, condition as Record<string, unknown>))
          })
        }
        continue
      }
      ...
      if (value === null) {
        ...
        // Handle filter object with operators
        for (const [operator, operatorValue] of Object.entries(value)) {
          switch (operator) {
            ...
            case 'not':
              if (operatorValue === null) {
                currentQuery = currentQuery.where(qualifiedField, 'is not', null)
              } else if (typeof operatorValue === 'object' && operatorValue !== null) {
                // Nested filter: not: { contains: 'foo' }
                // For simplicity in Phase 1, we'll just negate the value
                currentQuery = currentQuery.where(qualifiedField, '!=', this.transformWhereValue(field, operatorValue))
              } else {
                currentQuery = currentQuery.where(qualifiedField, '!=', this.transformWhereValue(field, operatorValue))
              }
  ...
  private applyWhereConditionsToBuilder(builder: any, where: Record<string, unknown>): any {
    ...
    for (const [field, value] of Object.entries(where)) {
      ...
      if (value === null) {
        currentBuilder = currentBuilder.where(qualifiedField, 'is', null)
      } else if (typeof value === 'object' && value !== null && !Array.isArray(value)) {
        for (const [operator, operatorValue] of Object.entries(value)) {
          switch (operator) {
            case 'equals':
              currentBuilder = currentBuilder.where(qualifiedField, '=', this.transformWhereValue(field, operatorValue))
              break
            case 'gt':
              currentBuilder = currentBuilder.where(qualifiedField, '>', this.transformWhereValue(field, operatorValue))
              break
            ...
            case 'not':
              ...
              break
            case 'in':
              currentBuilder = currentBuilder.where(qualifiedField, 'in', (operatorValue as unknown[]).map(v => this.transformWhereValue(field, v)))
              break
            case 'notIn':
              currentBuilder = currentBuilder.where(qualifiedField, 'not in', (operatorValue as unknown[]).map(v => this.transformWhereValue(field, v)))
              break
            case 'contains':
              currentBuilder = currentBuilder.where(qualifiedField, 'like', `%${operatorValue}%`)
              break
            case 'startsWith':
              currentBuilder = currentBuilder.where(qualifiedField, 'like', `${operatorValue}%`)
              break
        ...
        currentBuilder = currentBuilder.where(qualifiedField, '=', this.transformWhereValue(field, value))
      }
    }
