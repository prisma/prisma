...
- Integer representation presets: `field.bigIntNumber()` (Postgres emits `pg/int8number@1`, SQLite `sqlite/bigintnumber@1`) reads and writes a 64-bit integer column as a JS `number`, throwing outside ±(2^53 − 1); `field.unboundedInt()` (Postgres only, emits `pg/unboundedint@1`) reads and writes an unconstrained `numeric` as an exact `bigint`. Bare `field.bigint()` keeps the lossless `pg/int8@1`. See the [codec authoring guide](../../../../docs/reference/codec-authoring-guide.md#the-integer-representation-presets).
