...
- Integer representation types: PostgreSQL and SQLite expose `type.BigIntNumber()`; PostgreSQL also exposes `type.UnboundedInt()`. Register each result in the contract's returned `types` map and pass that same instance to `field.namedType(...)`, or use a direct per-codec column helper with `field.column(...)`. Bare `field.bigint()` keeps the lossless `pg/int8@1`. See the [codec authoring guide](../../../../docs/reference/codec-authoring-guide.md#the-integer-representation-types).
