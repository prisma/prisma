...
async function main() {
  ...
  const prisma = new PrismaClient({
    adapter,
    comments: [appPlugin, contextPlugin, tracingPlugin],
  ...
  const expectedQueries = [
    // findMany
    "SELECT `main`.`User`.`id`, `main`.`User`.`email`, `main`.`User`.`name` FROM `main`.`User` WHERE 1=1 LIMIT ? OFFSET ? /*action='findMany',application='sqlcommenter-e2e-test',model='User',queryType='single',source='tracing-plugin',traceId='test-trace-123',version='1.0.0'*/",
    // create - INSERT
    "INSERT INTO `main`.`User` (`email`, `name`) VALUES (?,?) RETURNING `id` AS `id`, `email` AS `email`, `name` AS `name` /*action='createOne',application='sqlcommenter-e2e-test',model='User',queryType='single',source='tracing-plugin',traceId='test-trace-123',version='1.0.0'*/",
    // findFirst
    "SELECT `main`.`User`.`id`, `main`.`User`.`email`, `main`.`User`.`name` FROM `main`.`User` WHERE `main`.`User`.`email` = ? LIMIT ? OFFSET ? /*action='findFirst',application='sqlcommenter-e2e-test',model='User',queryType='single',source='tracing-plugin',traceId='test-trace-123',version='1.0.0'*/",
  ]
