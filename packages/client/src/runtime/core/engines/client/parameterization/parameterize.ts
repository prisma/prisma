...
export function parameterizeQuery(query: JsonQuery, view: ParamGraphView): ParameterizeResult {
  ...
  const parameterizedQuery: JsonQuery = {
    ...query,
    query: parameterizeFieldSelection(query.query, root?.a, root?.o, 'query', view, placeholders, valueToPlaceholder),
  }
...
function parameterizeValue(
  ...
  switch (classified.kind) {
    ...
    case 'primitive':
      return handlePrimitive(classified.value, edge, path, view, placeholders, valueToPlaceholder)

    case 'taggedScalar':
      return handleTaggedScalar(
        value as JsonInputTaggedValue,
        classified.tag,
        edge,
        path,
        placeholders,
    ...
    case 'array':
      return handleArray(classified.items, value, edge, path, view, placeholders, valueToPlaceholder)

    case 'object':
      return handleObject(classified.entries, edge, path, view, placeholders, valueToPlaceholder)
...
function handlePrimitive(
  ...
  if (hasFlag(edge, EdgeFlag.ParamEnum) && edge.e !== undefined && typeof value === 'string') {
    ...
    if (enumValues?.includes(value)) {
      ...
      const existingPath = valueToPlaceholder.get(valueKey)

      if (existingPath !== undefined) {
        return createPlaceholder(existingPath, type)
      }

      valueToPlaceholder.set(valueKey, path)
...
function handleTaggedScalar(
  ...
  const valueKey = createValueKey(decoded, type)
  const existingPath = valueToPlaceholder.get(valueKey)
...
function handleArray(
  ...
  if (hasFlag(edge, EdgeFlag.ParamScalar) && getScalarMask(edge) & ScalarMask.Json) {
    ...
    const valueKey = createValueKey(jsonValue, type)
    const existingPath = valueToPlaceholder.get(valueKey)
    ...
    valueToPlaceholder.set(valueKey, path)
    placeholders.set(path, jsonValue)
    return createPlaceholder(path, type)
  }
  ...
  if (hasFlag(edge, EdgeFlag.ParamEnum)) {
    ...
    if (enumValues && items.every((item) => typeof item === 'string' && enumValues.includes(item))) {
      ...
      const valueKey = createValueKey(items, type)
      const existingPath = valueToPlaceholder.get(valueKey)
  ...
  if (hasFlag(edge, EdgeFlag.ParamListScalar)) {
    ...
    if (allValid && items.length > 0) {
      ...
      const valueKey = createValueKey(decodedItems, type)
      const existingPath = valueToPlaceholder.get(valueKey)
    ...
    if (childNode) {
      return items.map((item, i) => {
        if (isPlainObject(item)) {
          return parameterizeObject(item, childNode, `${path}[${i}]`, view, placeholders, valueToPlaceholder)
        }
        return item
      })
    }
...
function handleObject(
  ...
  if (hasFlag(edge, EdgeFlag.Object)) {
    const childNode = view.inputNode(edge.c)
    if (childNode) {
      return parameterizeObject(obj, childNode, path, view, placeholders, valueToPlaceholder)
    }
  }
