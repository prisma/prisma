...
import { parameterizeBatchWithSchema, parameterizeQueryWithSchema } from './traverse'

describe('parameterizeQueryWithSchema', () => {
    ...
    i: [
      // Node 0: UserWhereInput
      {
        f: {
          ...
          11: { k: EdgeFlag.ParamScalar, m: ScalarMask.String, e: 0 }, // status (enum)
        },
      },
      // Node 1: StringFilter
      {
        f: {
          ...
          6: { k: EdgeFlag.ListScalar, m: ScalarMask.String }, // in
        },
      },
  ...
  describe('basic parameterization', () => {
    it('parameterizes scalar values in where clause', () => {
      ...
      const result = parameterizeQueryWithSchema(query, view)
      ...
      expect(result.parameterizedQuery.query.arguments).toEqual({
        where: { name: { $type: 'Param', value: 'query.arguments.where.name' } },
      })
    })
  ...
  describe('tagged scalar values', () => {
    it('parameterizes DateTime tagged values', () => {
      ...
      const result = parameterizeQueryWithSchema(query, viewWithDateTime)
  ...
  describe('cache key consistency', () => {
    it('generates same parameterized query for different values with same structure', () => {
      ...
      const result1 = parameterizeQueryWithSchema(query1, view)
      const result2 = parameterizeQueryWithSchema(query2, view)
    ...
    it('maintains stable key order in objects', () => {
      const query1: JsonQuery = {
        modelName: 'User',
        action: 'findMany',
        query: {
      ...
      const query2: JsonQuery = {
        ...
        action: 'findMany',
        query: {
          arguments: { where: { email: 'different@example.com', id: '456' } },
          selection: { $scalars: true },
        },
      }
...
describe('parameterizeBatchWithSchema', () => {
  const sampleGraph: ParamGraph = {
    s: ['where', 'id', 'email'],
    en: [],
    i: [
  ...
  it('parameterizes all queries in a batch with unique placeholder names', () => {
    ...
    const result = parameterizeBatchWithSchema(batch, view)
  ...
  it('generates consistent cache keys for batches with same structure', () => {
    ...
    const result1 = parameterizeBatchWithSchema(batch1, view)
    const result2 = parameterizeBatchWithSchema(batch2, view)
