...
export function checkPlatformCaching({ postinstall, ciName, clientVersion }: Config) {
  ...
  if (ciName && ciName in cachingPlatforms) {
    const message =
      `Prisma has detected that this project was built on ${ciName}, which caches dependencies. This leads to an outdated Prisma Client because Prisma's auto-generation isn't triggered. To fix this, make sure to run the \`prisma generate\` command during the build process.
