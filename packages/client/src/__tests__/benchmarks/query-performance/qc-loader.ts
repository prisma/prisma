...
import { CompilerWasmLoadingConfig, QueryCompilerConstructor } from '@prisma/client-common'

import { wasmQueryCompilerLoader } from '../../../runtime/core/engines/client/WasmQueryCompilerLoader'

export function getQueryCompilerWasmConfig(provider: string): CompilerWasmLoadingConfig {
...
export function loadQueryCompiler(provider: string): Promise<QueryCompilerConstructor> {
  return wasmQueryCompilerLoader.loadQueryCompiler({
    ...
    clientVersion: '0.0.0',
    compilerWasm: getQueryCompilerWasmConfig(provider),
  })
}
