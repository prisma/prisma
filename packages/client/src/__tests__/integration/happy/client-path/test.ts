...
const projectDir = path.join(__dirname, '__helpers__')
...
beforeAll(async () => {
  await generateInFolder({
    ...
    useOutputFromSchema: true,
  })
})

test('custom client path', async () => {
  process.chdir(projectDir)
  const { PrismaClient, Prisma } = require('./__helpers__/server/client')
  ...
  const rawQueryTemplateWithParams = await db.$queryRaw`SELECT *
                                                          FROM User
                                                          WHERE name = ${'Alice'}`
  ...
  const rawQueryTemplateFromSqlTemplate = await db.$queryRaw(
    sql`
            ...
            FROM ${raw('User')} ${sql`WHERE name = ${'Alice'}`} ${empty}
        `,
  )
  ...
  // Test $executeRaw
  const $executeRawTemplate = await db.$executeRaw`UPDATE User
                                                     SET name = ${'name'}
                                                     WHERE id = ${'id'}`
