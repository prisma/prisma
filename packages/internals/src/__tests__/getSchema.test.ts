...
import { getSchemaWithPath, SchemaPathFromConfig } from '../cli/getSchema'
...
it('throws error if prisma.config.ts with single file is used but schema file cannot be found', async () => {
  ...
  expect(res).toMatchInlineSnapshot(
    `[Error: Could not load schema from file \`db/schema.prisma\` provided by "prisma.config.ts" config\`: file not found]`,
  )
})
...
it('throws error if prisma.config.ts with folder is used but schema files cannot be found', async () => {
  ...
  expect(res).toMatchInlineSnapshot(
    `[Error: Could not load schema from folder \`db\` provided by "prisma.config.ts" config\`: directory not found]`,
  )
})
