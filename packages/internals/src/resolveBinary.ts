...
async function getBinaryName(name: EngineNameEnum): Promise<string> {
  const platform = await getPlatform()
  const extension = platform === 'windows' ? '.exe' : ''

  if (name === EngineNameEnum.libqueryEngine) {
...
export { EngineNameEnum as BinaryType }
export async function resolveBinary(name: EngineNameEnum, proposedPath?: string): Promise<string> {
