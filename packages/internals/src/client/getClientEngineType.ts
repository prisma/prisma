...
export function getClientEngineType(generatorConfig?: GeneratorConfig): ClientEngineType {
  const engineTypeFromEnvVar = getEngineTypeFromEnvVar(generatorConfig)
  ...
  if (generatorConfig?.config.engineType === ClientEngineType.Library) {
    ...
    generatorConfig?.config.engineType === ClientEngineType.Wasm &&
    generatorConfig?.previewFeatures.includes('driverAdapters')
  ) {
...
function getEngineTypeFromEnvVar(generatorConfig?: GeneratorConfig) {
  const engineType = process.env.PRISMA_CLIENT_ENGINE_TYPE
  if (engineType === ClientEngineType.Library) {
    return ClientEngineType.Library
  } else if (engineType === ClientEngineType.Binary) {
    return ClientEngineType.Binary
  } else if (engineType === ClientEngineType.Wasm && generatorConfig?.previewFeatures.includes('driverAdapters')) {
    return ClientEngineType.Wasm
  } else {
    return undefined
  }
