...
- **Contract provider**: `mongoContract()` (exported from `./provider`) integrates with the CLI's `prisma orm contract emit` command, reading a `.prisma` schema file and producing a `ContractConfig`
