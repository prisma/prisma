...
import { ErrorArea, getWasmError, isWasmPanic, RustPanic, SchemaContext, wasm } from '@prisma/internals'
...
// TODOs:
// - pass "initial schema" from `SchemaContext` to `SchemaEngineWasm`
// - change `wasm.SchemaEngineWasm`'s constructor to an async static method?
// - implement `getDatabaseDescription`
// - re-throw `RustPanic`
// - re-throw any error
...
export class SchemaEngineWasm {
  ...
  static async setup({ adapter, ...rest }: SchemaEngineWasmSetupInput): Promise<SchemaEngineWasm> {
    ...
    const engine = await new wasm.SchemaEngineWasm(adapter)
