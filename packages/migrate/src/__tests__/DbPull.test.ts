...
describe('postgresql views fs I/O', () => {
  ...
  const computeSetupParams = (variant?: string) => {
    const fixturePath = path.join('introspection', 'postgresql', `views-fs${variant ? variant : ''}`)
  ...
  const setupPostgresForViewsIO = (variant?: string) => {
  ...
  describe('with preview', () => {
    ...
    test('introspection creates view definition files', async () => {
      ...
      const workWorkersView = await ctx.fs.readAsync('views/work/workers.sql')
    ...
    test('extraneous files in views folder should be removed on db pull', async () => {
      ctx.fixture(path.join(fixturePath))
  ...
  describe('no preview', () => {
    ...
    test('introspection without preview feature', async () => {
      ctx.fixture(path.join(fixturePath))
