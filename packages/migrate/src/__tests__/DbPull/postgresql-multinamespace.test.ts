...
describeMatrix(postgresOnly, 'postgresql-multinamespace', () => {
  const connectionString = process.env.TEST_POSTGRES_URI_MIGRATE!.replace(
    'tests-migrate',
    'tests-migrate-db-pull-multinamespace-postgresql',
  )
  ...
  test('--url --schemas=base (1 existing schema) should succeed', async () => {
    ...
    expect(ctx.normalizedCapturedStdout()).toMatchInlineSnapshot(`
      ...
      datasource db {
        provider   = "postgresql"
        url        = "postgres://prisma:prisma@localhost:5432/tests-migrate-db-pull-multinamespace-postgresql"
        namespaces = ["base"]
      }
  ...
  test('--url with `?schema=base` should succeed', async () => {
    ...
    expect(ctx.normalizedCapturedStdout()).toMatchInlineSnapshot(`
      "datasource db {
        provider = "postgresql"
        url      = "postgres://prisma:prisma@localhost:5432/tests-migrate-db-pull-multinamespace-postgresql?schema=base"
      }
