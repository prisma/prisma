...
describe('db execute', () => {
  describe('generic', () => {
    ...
    it('should fail if missing --file and --stdin', async () => {
      ...
      const result = dbExecute.parse(['--preview-feature'])
      await expect(result).rejects.toThrowErrorMatchingInlineSnapshot(`
    ...
    it('should fail if both --file and --stdin are provided', async () => {
      ...
      const result = dbExecute.parse(['--preview-feature', '--file=1', '--stdin'])
      await expect(result).rejects.toThrowErrorMatchingInlineSnapshot(`
    ...
    it('should fail if missing --schema and --url', async () => {
      ...
      const result = dbExecute.parse(['--preview-feature', '--file=1'])
      await expect(result).rejects.toThrowErrorMatchingInlineSnapshot(`
    ...
    it('should fail if both --schema and --url are provided', async () => {
      ...
      const result = dbExecute.parse(['--preview-feature', '--stdin', '--schema=1', '--url=1'])
      await expect(result).rejects.toThrowErrorMatchingInlineSnapshot(`
    ...
    it('should fail if --file does no exists', async () => {
      ...
      try {
        await dbExecute.parse(['--preview-feature', '--file=./doesnotexists.sql', '--schema=1'])
      } catch (e) {
    ...
    it('should fail if --schema does no exists', async () => {
      ...
      try {
        await dbExecute.parse(['--preview-feature', '--file=./script.sql', '--schema=./doesnoexists.schema'])
      } catch (e) {
        expect(e.code).toEqual(undefined)
  ...
  describe('mongodb', () => {
    it('should fail with not supported error with --file --schema', async () => {
      ...
      const result = dbExecute.parse(['--preview-feature', '--schema=./prisma/schema.prisma', '--file=./script.js'])
      await expect(result).rejects.toThrowErrorMatchingInlineSnapshot(`
...
DROP TABLE 'test-dbexecute';`
    ...
    it('should pass if no schema file in directory with --file --url', async () => {
      ...
      const result = dbExecute.parse(['--preview-feature', '--url=file:./dev.db', '--file=./script.sql'])
    ...
    it('should pass with --file --schema', async () => {
      ...
      fs.writeFileSync('script.sql', sqlScript)
      const result = dbExecute.parse(['--preview-feature', '--schema=./prisma/schema.prisma', '--file=./script.sql'])
      await expect(result).resolves.toMatchInlineSnapshot(`Script executed successfully.`)
    })
...
COMMIT;`,
    ...
    it('should fail with P1013 error with invalid url with --file --url', async () => {
      ...
      fs.writeFileSync('script.sql', '-- empty')
      try {
        await dbExecute.parse(['--preview-feature', '--url=invalidurl', '--file=./script.sql'])
      } catch (e) {
    ...
    it('should pass with --file --url=file:doesnotexists.db', async () => {
      ...
      fs.writeFileSync('script.sql', sqlScript)
      const result = dbExecute.parse(['--preview-feature', '--url=file:doesnotexists.db', '--file=./script.sql'])
      await expect(result).resolves.toMatchInlineSnapshot(`Script executed successfully.`)
    })
    ...
    it('should fail with P1014 error with --file --schema', async () => {
      ...
      try {
        await dbExecute.parse(['--preview-feature', '--schema=./prisma/schema.prisma', '--file=./script.sql'])
      } catch (e) {
    ...
    it('should fail with invalid SQL error from database with --file --schema', async () => {
      ...
      await expect(result).rejects.toThrowError(
        expect.objectContaining({
          message: expect.stringContaining('near "ThisisnotSQL": syntax error'),
        }),
      )
    })
  })
...
COMMIT;`,
    ...
    it('should pass with --file --url', async () => {
      ...
      fs.writeFileSync('script.sql', sqlScript)
      const result = dbExecute.parse(['--preview-feature', '--url', connectionString, '--file=./script.sql'])
      await expect(result).resolves.toMatchInlineSnapshot(`Script executed successfully.`)
    })
    ...
    it('should fail if DROP DATABASE with --file --schema', async () => {
      ...
      await expect(result).rejects.toThrowError(
        expect.objectContaining({
          message: expect.stringContaining('ERROR: DROP DATABASE cannot'),
        }),
      )
    })
    ...
    it('should fail with P1013 error with invalid url provider with --file --url', async () => {
      ...
      const result = dbExecute.parse(['--preview-feature', '--url=invalidurl', '--file=./script.sql'])
      await expect(result).rejects.toThrowError(
        expect.objectContaining({
          code: 'P1013',
          message: expect.stringContaining('invalidurl'),
        }),
      )
    })
    ...
    // https://github.com/prisma/prisma/issues/11673
    it('should fail with P1003 error with --file --schema', async () => {
      ...
      await expect(result).rejects.toThrowError(
        expect.objectContaining({
          code: 'P1003',
          message: expect.stringContaining('does not exist on the database server'),
        }),
      )
    })
...
COMMIT;`,
    ...
    it('should fail with P1013 error with invalid url with --file --url', async () => {
      ...
      await expect(result).rejects.toThrowError(
        expect.objectContaining({
          code: 'P1013',
          message: expect.stringContaining('invalid port number'),
        }),
      )
    })
    it('should fail with P1013 error with invalid url provider with --file --url', async () => {
...
DROP DATABASE "test-dbexecute";`
    ...
    it('should pass with --file --url', async () => {
      ...
      fs.writeFileSync('script.sql', sqlScript)
      const result = dbExecute.parse(['--preview-feature', '--url', jdbcConnectionString, '--file=./script.sql'])
      await expect(result).resolves.toMatchInlineSnapshot(`Script executed successfully.`)
    })
