...
export function errorInitMissingFlags(options: {
  ...
  return new CliStructuredError('CLI.INIT_MISSING_FLAGS', 'Missing required flags', {
    ...
    fix: `Re-run with the missing flag(s) supplied, e.g. \`prisma init --yes ${fixList}\`. Use \`prisma init --help\` to see every flag.`,
...
export function errorInitUserAborted(): CliStructuredError {
  return new CliStructuredError('CLI.INIT_USER_ABORTED', 'Init cancelled', {
    ...
    fix: 'Re-run `prisma init` and complete the prompts, or pass the required inputs as flags (see `--help`) for a non-interactive run.',
    severity: 'info',
  });
}
...
export function errorInitInvalidManifest(options: {
  ...
  return new CliStructuredError('CLI.INIT_INVALID_MANIFEST', `Failed to parse ${options.path}`, {
    ...
    fix: `Fix the JSON syntax in \`${options.path}\` (a missing comma or unbalanced brace is the most common cause), then re-run \`prisma init\`.`,
...
export function errorInitInvalidTsconfig(options: {
  ...
  return new CliStructuredError('CLI.INIT_INVALID_TSCONFIG', `Failed to parse ${options.path}`, {
    ...
    fix: `Fix the syntax in \`${options.path}\` and re-run \`prisma init\`. \`init\` accepts JSONC (comments and trailing commas) but cannot recover from unbalanced braces or missing commas.`,
...
export function errorInitWriteFailed(options: {
  ...
  return new CliStructuredError('CLI.INIT_WRITE_FAILED', `Failed to write ${options.path}`, {
    ...
    fix: 'Fix what stopped the write — a directory sitting where the file goes, permissions, a full disk — then run `prisma init` again. It will ask you to confirm replacing the files this run already wrote (listed in `meta.filesWritten`).',
 ...
 * run the install command manually, or re-run `init --skip-skills` to
...
export function errorInitSkillInstallFailed(options: {
  ...
  return new CliStructuredError(
    ...
    'Failed to install Prisma Next skills',
    {
      ...
      fix:
        'Either:\n' +
        '  - Re-run `prisma init --skip-skills` to skip the skill install for this run, or\n' +
