...
describe('migrate', () => {
  it('settles as a completed envelope carrying the apply document', async () => {
    ...
    const run = await harness(ormConfig(cwd)).run(['orm', 'migrate', '--json'], { cwd });
  ...
  it('lays the applied spaces out as a tree the engine draws', async () => {
    ...
    const run = await harness(ormConfig(cwd)).run(['orm', 'migrate'], {
      cwd,
      isTty: { stdout: true },
    });
  ...
  it('takes the connection from --db over the config', async () => {
    ...
    await harness(ormConfig(cwd)).run(['orm', 'migrate', '--db', 'postgres://other/db', '--json'], {
      cwd,
    });
  ...
  it('errors when no connection is configured', async () => {
    ...
    const run = await harness(ormConfig(cwd, { db: undefined })).run(['orm', 'migrate', '--json'], {
      cwd,
    });
  ...
  describe('--show', () => {
    it('previews the route without applying anything', async () => {
      ...
      const run = await harness(ormConfig(cwd)).run(['orm', 'migrate', '--show', '--json'], {
        cwd,
      });
    ...
    it('ships the topology as a drawing whose spans carry tone', async () => {
      ...
      const run = await harness(ormConfig(cwd)).run(['orm', 'migrate', '--show'], {
        cwd,
        isTty: { stdout: true },
      });
      const blocks = run.presented?.presentation.human ?? [];
    ...
    it('names the from-state and the target in the header', async () => {
      ...
      const run = await harness(ormConfig(cwd)).run(
        ['orm', 'migrate', '--show', '--from', C1, '--to', C2],
        {
          cwd,
          isTty: { stdout: true },
        },
