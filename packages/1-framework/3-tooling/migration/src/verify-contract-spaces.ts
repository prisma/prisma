  ...
  for (const spaceId of [...inputs.loadedSpaces].sort()) {
    ...
    if (!inputs.spaceDirsOnDisk.includes(spaceId)) {
      violations.push({
        ...
        spaceId,
        remediation: `Extension '${spaceId}' is declared in extensions but has not been emitted; run \`prisma orm migrate\`.`,
      });
      continue;
    }
    ...
    if (head.hash !== marker.hash) {
      violations.push({
        ...
        remediation: `Marker row for space '${spaceId}' is keyed at ${marker.hash}, but the on-disk ${join('migrations', spaceId, 'contract.json')} resolves to ${head.hash}. Run \`prisma orm db update\` to advance the database, or \`prisma orm migrate\` if the descriptor was bumped without re-emitting.`,
      });
      continue;
    }
    ...
    if (missing.length > 0) {
      violations.push({
        ...
        remediation: `Marker row for space '${spaceId}' is missing invariants [${missing.map((s) => JSON.stringify(s)).join(', ')}]. Run \`prisma orm db update\` to apply the corresponding data-transform migrations.`,
      });
    }
  }
