...
export const postgresAggregateDescriptors: ReadonlyArray<SqlAggregateDescriptor> = [
  // `count` returns `bigint` whether it counts entries or non-null values, which is what makes it input-agnostic rather than merely input-less. A row count is a `number` to a JS developer, so that is what the bare operation reads it as — and past 2^53 it throws rather than answer with a rounded tally.
  {
    operation: 'count',
    input: { kind: 'any' },
