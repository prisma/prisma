...
It is dev-only tooling, not a runtime dependency — extension authors install it to verify their own codec descriptors the same way the built-in PostgreSQL adapter suite verifies its own.
...
The harness, case types, and runner are the public API. It does not ship the built-in PostgreSQL adapter's own case catalogue — that stays with the adapter's test suite as its own coverage, not general-purpose vocabulary.
