-- Supabase reference schema fixture — captured from a live local Supabase
-- stack via `pg_dump --schema-only --no-owner`, all schemas (auth, storage,
-- realtime, _realtime, vault, net, supabase_functions, extensions, graphql,
-- graphql_public, public).
--
-- Version pins (record alongside any refresh): supabase/postgres:17.6.1.106
-- (PostgreSQL 17.6), gotrue v2.188.1, storage-api v1.54.1, supabase CLI
-- 2.95.4. Captured 2026-07-12.
--
-- Restored into a fresh PGlite dev database by `restore.ts` in this
-- directory (see also `roles.sql`, which must be restored first — policies
-- and grants below reference anon/authenticated/service_role/etc.).
--
-- Trims — PGlite cannot execute the following, removed with this fixture
-- verified restorable via the auto-trim loop in
-- packages/3-extensions/supabase/scripts/_build-fixture.mjs (deleted after
-- use; rerun a similar loop when refreshing this fixture):
--
--   1. `CREATE EXTENSION` for pg_net, pg_graphql, pg_stat_statements,
--      pgcrypto, supabase_vault ("extension ... is not available" — PGlite
--      ships none of these) — plus every GRANT/COMMENT statement that only
--      makes sense once one of those five extensions has run: objects under
--      `net.*`, the pgcrypto function family in `extensions.*` (armor/crypt/
--      digest/encrypt/gen_random_bytes/gen_salt/hmac/pgp_*), the
--      `pg_stat_statements` table+functions, `vault.secrets`/
--      `vault.decrypted_secrets`/`vault._crypto_aead_det_decrypt`/
--      `vault.create_secret`/`vault.update_secret`, and
--      `graphql_public.graphql(...)` (only materializes when pg_graphql's
--      install-time event trigger fires). `uuid-ossp` restores cleanly and
--      is kept.
--   2. The `realtime.messages_2026_07_11` .. `_15` daily partition children
--      (table, ATTACH PARTITION, primary key, index, index ATTACH
--      PARTITION, grants) — trips two collisions unrelated to extensions:
--      Postgres auto-attaches each partition's PK index to the parent's
--      partitioned index as soon as the partition's own PK constraint is
--      added, so the dump's later explicit `ALTER INDEX ... ATTACH
--      PARTITION` is redundant and errors ("cannot attach index ... already
--      a partition"); separately, the `@prisma/dev` PGlite harness installs
--      its own per-table WAL-capture trigger on `CREATE TABLE`, and Postgres
--      independently propagates the partitioned parent's triggers onto a
--      newly attached partition, producing a duplicate ("trigger ...
--      already exists"). The parent `realtime.messages` (partitioned, zero
--      partitions) is kept — `realtime` is an undeclared/tolerated schema
--      for this pack's contract either way.
--
-- Nothing under `auth.*` or `storage.*` — the two schemas this pack
-- declares — was trimmed.
