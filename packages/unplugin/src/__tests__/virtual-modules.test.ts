...
describe('VirtualModuleResolver', () => {
  ...
  it('should identify virtual modules', () => {
    expect(resolver.isVirtualModule('.ork/types')).toBe(true)
    expect(resolver.isVirtualModule('virtual:ork/types')).toBe(true)
    expect(resolver.isVirtualModule('regular-module')).toBe(false)
  })

  it('should resolve virtual IDs correctly', () => {
    expect(resolver.resolveVirtualId('.ork/types')).toBe('virtual:ork/types')
    expect(resolver.resolveVirtualId('virtual:ork/types')).toBe('virtual:ork/types')
    expect(resolver.resolveVirtualId('regular-module')).toBeNull()
  })

  it('should extract module keys', () => {
    expect(resolver.getModuleKey('virtual:ork/types')).toBe('types')
    expect(resolver.getModuleKey('virtual:ork/index')).toBe('index')
  })
})
...
describe('VirtualTypeGenerator', () => {
  ...
  it('should generate types module', () => {
    ...
    expect(result).toContain('Generated by unplugin-ork')
