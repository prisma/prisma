    ...
    it('should process small schemas quickly', async () => {
      const result = await benchmark(
        ...
        async () => {
          ...
          if (load) {
            load('virtual:refract/types')
            load('virtual:refract/index')
            load('virtual:refract/generated')
          }
        },
        5,
      )
      ...
      const result = await benchmark(
        ...
        () => {
          ...
          if (resolveId) {
            ...
            resolveId('.refract/types')
            resolveId('.refract/index')
            resolveId('.refract/generated')
            resolveId('virtual:refract/types')
            resolveId('virtual:refract/index')
      ...
      const result = await benchmark(
        ...
        async () => {
          ...
          if (load) {
            ...
            const promises = [
              Promise.resolve(load('virtual:refract/types')),
              Promise.resolve(load('virtual:refract/index')),
              Promise.resolve(load('virtual:refract/generated')),
              Promise.resolve(load('virtual:refract/types')), // Duplicate request
              Promise.resolve(load('virtual:refract/index')), // Duplicate request
            ]
