...
class BetterSQLite3Queryable<ClientT extends StdClient> implements SqlQueryable {
  ...
  constructor(
    ...
    protected readonly driverOptions?: PrismaBetterSqlite3Options,
  ) {}
  ...
  private executeIO(query: SqlQuery): Promise<BetterSQLite3Meta> {
    try {
      const args = query.args.map((arg, i) => mapArg(arg, query.argTypes[i], this.driverOptions))
...
class BetterSQLite3Transaction extends BetterSQLite3Queryable<StdClient> implements Transaction {
  ...
  constructor(
    ...
    driverOptions: PrismaBetterSqlite3Options | undefined,
    unlockParent: () => void,
  ) {
    super(client, driverOptions)
...
export class PrismaBetterSqlite3Adapter extends BetterSQLite3Queryable<StdClient> implements SqlDriver {
  #mutex = new Mutex()

  constructor(client: StdClient, driverOptions?: PrismaBetterSqlite3Options) {
    super(client, driverOptions)
  }
  ...
  async startTransaction(isolationLevel?: IsolationLevel): Promise<Transaction> {
    ...
    try {
      ...
      return new BetterSQLite3Transaction(this.client, options, this.driverOptions, release)
    } catch (e) {
      this.onError(e)
    }
  }
...
export class PrismaBetterSqlite3DriverFactory implements SqlMigrationAwareDriverFactory {
  readonly provider = 'sqlite'
  readonly driverName = packageName
