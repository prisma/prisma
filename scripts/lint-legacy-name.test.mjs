...
describe('what the check forbids', () => {
  it('fails on the old package scope, naming the file and line', () => {
    write('src/db.ts', "import { orm } from '@internal/sql-orm-client';\n");
    const result = run();
  ...
  it('lists every allowance when it fails, so the reader can see which ones exist', () => {
    write('src/db.ts', "import x from '@internal/contract';\n");
    const result = run();
...
describe('what the check allows', () => {
  ...
  it('allows the changelog and the release notes', () => {
    write('CHANGELOG.md', 'Published as `@internal/cli` in v0.16.0.\n');
    write('docs/releases/v0.16.0.md', 'Upgrade `@internal/postgres` to 0.16.\n');
    const result = run();
    assert.equal(result.status, 0, `expected exit 0; stderr=${result.stderr}`);
  });

  it('allows ADRs and the project and drive write-ups', () => {
    write('docs/architecture docs/adrs/ADR 211 - shim.md', 'The `@internal/cli` dist is copied.\n');
    write('projects/some-project/spec.md', 'Depends on `@internal/emitter`.\n');
