...
`src/db.ts` wires `@internal/middleware-cache` into the Mongo runtime — the same middleware used by `examples/prisma-next-demo` against Postgres. The package depends only on `@internal/framework-components/runtime`; cache keys come from `RuntimeMiddlewareContext.contentHash(exec)`, which `MongoRuntimeImpl` populates the same way `SqlRuntime` does, so the middleware works against Mongo out of the box.
