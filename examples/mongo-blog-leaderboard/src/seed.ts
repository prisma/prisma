...
export async function seed(orm: Db['orm']) {
  ...
  await articles.createAll([
    ...
    {
      title: 'Pipelines for Reporting',
      content: 'Aggregation pipelines unlock dashboards and leaderboards without a separate ETL.',
      summary: 'Reporting straight from the operational store.',
      authorId: alice._id,
  ...
  await tutorials.createAll([
    ...
    {
      title: 'Pipelines, Deeply',
      content: 'A deeper look at $group, $lookup, and $facet for analytical workloads.',
      difficulty: 'advanced',
      duration: 60,
      authorId: bob._id,
