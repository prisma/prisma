...
**Decided (operator, 2026-07-31): facade-only.** A consuming app depends on exactly one facade package plus any extension packs it chooses — no `@prisma/orm-family-sql`, no `@prisma/orm-toolchain` in an app's dependencies. The CLI reaches apps through the facade's launcher bin. ADR 242 is amended to say so.

Implementation (TML-3128) is driven by evidence rather than by the scan: rewrite one example per family using only its facade plus extension packs, and add a facade entrypoint for each import with no facade path. Some scanned imports are stale example code, not real gaps — `prisma-next-demo` takes the generic `Runtime` from `sql-runtime` where the facade already offers `PostgresRuntime` — and those get fixed in the example instead.

Consumer trees are held to this by two lints: `lint-single-import-root.mjs` (a consumer never names both roots — hard fail, correctness) and a high-water-mark ratchet on internal imports in `examples/`, `apps/`, `test/` (TML-3132), whose target is zero.
