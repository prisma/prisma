...
Complete the CLI-owned `prisma7` identity slice by making identity selection explicit at CLI composition boundaries and proving the shipped behavior from packed artifacts. The slice stays inside `packages/cli`: stable Prisma domain names remain unchanged, and lower-package identity work is still follow-up scope.
...
- **Tests (evidence)**:
  ...
  - [packages/client/tests/e2e/prisma7-compatibility/tests/**snapshots**/main.test.ts.snap](packages/client/tests/e2e/prisma7-compatibility/tests/__snapshots__/main.test.ts.snap)
...
3. Replace slice-local mock evidence with one packed installed-artifact snapshot, while existing ordinary-Prisma tests keep the explicit `'prisma'` call sites honest.
...
- **Packed compatibility proof now lives in one deterministic installed-artifact snapshot**: the slice removes four mock-heavy identity suites and the slice-added mock assertions, and instead snapshots real packed `prisma7` command output plus the generated init files.
  - **Why**: the reviewable proof should come from shipped commands, not constructor-level mock scaffolding.
  - **Implementation / evidence**:
    ...
    - [packages/client/tests/e2e/prisma7-compatibility/tests/**snapshots**/main.test.ts.snap](packages/client/tests/e2e/prisma7-compatibility/tests/__snapshots__/main.test.ts.snap) commits the command-visible proof.
...
- Snapshot normalization is intentionally narrow: it replaces cwd/temp/bin paths and projects `--version` output down to stable identity-bearing fields and metadata key/label sets.
