...
- **Packed proof replaces slice-local mock coverage**: the four mock-heavy identity suites and the slice-added mock assertions are gone. Evidence now comes from the single packed E2E in `packages/client/tests/e2e/prisma7-compatibility/tests/main.test.ts` plus its committed snapshot, which exercises real built/packed `prisma7 --help`, `validate --help`, text/JSON `--version`, `complete zsh`, `init`, `generate`, and `db push` flows. The snapshot only normalizes cwd/temp/bin paths and projects version output down to stable identity-bearing fields so the proof stays deterministic and reviewable.
