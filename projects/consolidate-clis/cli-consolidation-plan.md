...
This document defines the target experience for the Prisma 8 CLI. It describes where we are going, not what every repository supports today. It supersedes the v1 spec after review against the shipped surfaces of `prisma-cli`, `composer`, and `prisma-next`
...
- The classic Prisma (v7) implementation is not carried over, and there is no compatibility layer for it (Layer 6 below).
- Users get one prisma command and, when they use orchestration, one `prisma.config.ts`

The primary experience is local development to Prisma Cloud. Local-only work, external clouds (mainly composer focused), and direct resource management without Composer all stay supported. When a trade-off is unavoidable, we optimize for the local-to-Prisma-Cloud path (our own ecosystem).

Design decisions:

- Decided:
    - the data model noun is contract. Settled with the team; the schema-versus-contract question is closed.
- Decided:
    - the deployable unit is a **Service** (settled 2026-08-07); this spec says Service throughout.
...
- **Opinionated defaults, explicit off-ramps.** The happy path is strongly opinionated. Happy paths have a visible, supported way out.
- **Local first DX, Prisma Cloud preferred.** Local development needs no account. Prisma Cloud is the most integrated remote experience.
- **Adoption is progressive.** The ORM, migrations, a database, Compute, and full orchestration can each be adopted on their own.
- **Everyday commands use product nouns:** Project, Service, DB, Branch, Contract. Internals like ledgers, markers, and hashes stay inside advanced and recovery workflows.
...
- **Agents are first-class users.** Humans, agents, and CI get the same commands with the same behaviour.
...
└── Branches
    ...
    ├── staging (role: preview)
    └── feat-auth (role: preview)
```

A Branch is a named isolated copy of the Project’s runtime. One Branch can span providers:
...
| Project | The whole system described by one **prisma.config.ts**. Can hold several Services and Databases. A monorepo is one Project, not many. |
...
| Service | One deployable application or service. |
| Deployment | One build-and-release of an Service on a Branch. Immutable. |
| Database | A logical database used by one or more Services. |
...
`prisma.config.ts` is the single entry point for a Project. It can import definitions from other files (`services/web.ts`, `data/main.ts`). One config means one place where the system comes together, not one giant file.

The config holds what the user wants to exist: the Services and resources, how they connect, what they need from providers, and how local differs from remote. It never holds current cloud state: no resource IDs, no deployment status, no receipts, no credentials.
...
Three rules that must hold across all our tooling:

- **Evaluating the config does nothing.** It creates no resources and makes no network calls. It only describes the system.
- **Service code never imports the config.** The config imports service definitions, not the other way around.
- **An invalid config still loads.** Loading always succeeds and returns an inspectable result: here is what I was given, and here is why it is invalid. A broken config never breaks unrelated commands.
...
1. **A command's name tells you what it touches.** `project deploy` acts on the whole Project. `service deploy` ships one Service. `db migrate` changes one database. A command never quietly does more than its name says — no matter what is in the config.
...
4. **One config file, and reading it changes nothing.** `prisma.config.ts` is the only Project configuration. Evaluating it never creates a resource and never touches the network — it only describes the system. An invalid config still loads into an inspectable result that says why it is invalid; it never breaks unrelated commands.
5. **The resource commands are always available.** Composer is a convenience layer that drives the same commands under the hood — `service deploy`, `postgres create`, logs, and the rest. Using Composer never locks anyone out of them: inspecting and debugging the underlying resources directly always works. The config stays the source of intent, so what it declares is what the next `project deploy` converges the system toward. The same holds locally: `project dev` drives the same emulators the user can start, stop, and inspect directly.
6. **Create what's missing, and say so.** If a deploy needs a Project, Service, or Branch that doesn't exist yet, the CLI creates it and carries on — it never stops the user to do plumbing first. The rule is transparency, not restraint: the output always states plainly what was created on the user's behalf, and every command shows which Project, Branch, and Service it acted on.
7. **Local is local.** Local development runs emulators on the user's machine and has no effect on the cloud platform. Local is never a Branch and never a deploy target. Wiring a local service to a remote resource is possible, but only ever explicit.
...
13. **Every surface lands on the same target.** CLI, Console, GitHub, and agents all resolve to the same Project, Branch, and Service — the same repo never turns into two Projects because it was touched from two places. Resources are tracked by stable IDs, not display names or folder paths: renaming or moving code never creates a new resource, and when a target is ambiguous the CLI stops instead of picking one.
...
Examples: `prisma project deploy`, `prisma postgres backup restore`, `prisma service deployment logs`.
...
| **project** | The Project as a whole: check, dev, plan, deploy, plus Project records and env maps. | The unit that ties Services, Databases, migrations, and bindings together. |
...
| **service** | Build, run, deploy, and operate one Service. | Works on its own, without orchestration. |
| **db** | Work on the live database the config declares: migrate it, match it to the contract, query and seed it. | The connection world; account-free. |
| **postgres** | Create and administer hosted Prisma Postgres instances. | The admin world; authenticated, addressed by ID. |
| **migration** | Migration files on disk: plan, inspect, and manage refs. | The interrogative side of migrations; never changes a database. |
...
| **emulator** | Start, stop, and inspect the local emulators (postgres, compute, bucket). | The visible parts `project dev` runs; account-free. |
...
**Two worlds (OSS and Cloud), one rule.** Everything under `postgres`, `service`, `bucket`, `branch`, and `auth` needs a login and addresses resources by ID. Everything under `db`, `contract`, and `migration` works without an account, against the database the config declares. The tree itself tells you which commands need an account. `db` keeps the short form people have typed for years; it is the only abbreviated root.

## Who owns what in the data lifecycle

Four roots share the data lifecycle. One sentence each:

- **contract** owns the model: the files you author and the artefacts they produce.
- **migration** owns migration files on disk. It never changes a database; `status` and `log` may read one to report.
- **db** owns the live database through the configured connection. `db migrate` is the one command that advances it; `init`, `update`, `verify`, and `sign` work with its current state.
- **postgres** owns the hosted resource: instances, connections, backups, usage.

Migrate is active, migration is interrogative: the process versus the artefact.

## When a concept gets a root

A concept earns a root when it is the whole Project, shared context across resources, a resource that works on its own, an account-free workflow, or a cross-cutting integration. Everything else nests: `service deployment`, `postgres connection`, `postgres backup`, `bucket key`, `auth workspace`, `auth token`, `project env`, `migration ref`.
...
One narrow exception: `postgres`. It is the resource noun for a Prisma-hosted instance, the same way `bucket` is, not a product namespace. Admin commands only ever touch Prisma-managed resources, so `postgres` can only mean a Prisma Postgres instance; a Neon database is referenced or orchestrated, never administered here. If we ever host another engine, a `mongo` group follows the same pattern. The ban on org, brand, and provider names stays fully intact.

## Direct commands and orchestration

```
Direct commands                      Orchestration
├── prisma service deploy                └── prisma project deploy
...
`service deploy` ships one Service. `postgres create` creates one Database. `project deploy` brings the whole configured Project up to date. There is no bare `prisma deploy`; its scope would be unclear (invariant 1).
...
Services and preview Branches follow the same pattern: created when a deploy first targets them, always announced.
...
`project dev` runs everything locally by default. Under the hood it orchestrates one emulator per resource kind: a local Postgres, a local compute runtime, a local bucket store. The user can start, stop, and inspect each one directly through the `emulator` commands; `project dev` is a convenience on top of visible parts, not a black box (invariant 5). One local copy per working directory. Wiring a local service to a remote resource is possible, but always explicit and always shown before start.
...
This is directional, not a launch checklist.
...
- `service promote <deployment>` and `service rollback` sit flat. They are Service-level traffic actions that take a Deployment. `service deployment` keeps inspection: `list`, `show`, `logs`. Deployment logs also cover builds triggered by Git push, since those produce Deployments.
- `project env` manages the two env maps (`-role production|preview`) and per-Branch overrides (`-branch <name>`). `add` fails if the variable exists; `update` fails if it does not. Production values can be written but never read back; inspection shows metadata only. `pull` fetches preview values, explicitly, for local development. No output path ever prints a production value.
    ...
    this is based on current functionality. We are in the middle of adding service-scoped env variables which would change this behaviour. Branch specific overrides will most likely fall away
...
- `db init` and `db update` both make a running database match the contract without recording a migration; together they replace what `db push` did in the past. They differ in policy. `init` is strict: it sets up an empty or new database, and if it finds anything that does not match the contract it stops without changing anything. `update` is permissive: it updates whatever it finds and may change existing data, subject to the usual destructive-change checks. The name `sync` was rejected because it implies a two-way exchange that does not exist. `db verify` and `db sign` check and record where a database stands against the contract; together they replace classic failed-migration repair. `db schema` prints the live structure, read-only.
- `db migrate [--to <ref>]` is the one command that advances a live database through recorded migrations. Everything interrogative lives under `migration`. Migrate is active, migration is interrogative: the process versus the artifact.
- The `prisma migrate` miss gets a first-class error. Fifteen years of tutorials will type `prisma migrate ...`. The answer is a structured unknown-command error with "did you mean `prisma db migrate`". Guidance, never an alias (invariant 14). The quality of that message is a launch requirement.
...
- `init` detects and guides. It looks at what is in the repository, then runs a short guided setup to establish what the user is initializing, governed by flags. In automation the same flow is flag-driven, or fails with a structured error (invariant 12).
- `emulator` commands are a thin layer over emulators that already exist; `project dev` orchestrates the same ones. Useful on its own, and not on the critical path for consolidation.
- `auth token` manages workspace-scoped service tokens for CI and automation.

## Nested nouns

A child with its own identity gets a noun: `service deployment list`, `postgres connection create`, `postgres backup restore`. Never `list-deploys`, `create-connection`, or `restore-backup`.
...
| deploy | Make an Service or Project revision live. |
| migrate | Advance a live database through recorded migrations, forward only (`--to <ref>`). |
| init / update (in `db`) | The strict and permissive ways to make a database match the contract without recording a migration. |
| verify / sign | Check or record where a database stands against the contract. |
| start / stop | Start or stop a local emulator. |
| emit / infer | Produce the contract artifacts / derive a contract from a live database. |
| promote / rollback | Move Service traffic to a healthy Deployment / back to an earlier one. |
...
- `migration new` scaffolds a hand-written data migration. `migration graph` draws the graph itself, making the model visible. `migration ref` manages the named pointers. Refs stay inside the `migration` group.
...
The order: resolve the Branch and providers, create or update resources, apply migrations that pass their risk checks, resolve secrets and bindings, build and deploy Services, verify health, move traffic.

**Building.** `project deploy` builds Services with their build profiles by default. Bringing your own build stays first-class: an Service can point at a prebuilt artefact, and CI can hand one in. That is the Composer workflow of type-check, build, deploy. Every deploy records its build and its phases with the platform, so build history stays inspectable through `service deployment show` and `logs`. The API carries this; no separate build commands are needed.

Only what changed gets touched. An unchanged Service is not rebuilt or redeployed. Re-running a deploy is safe; it continues from recorded state (invariant 11).
...
There is no `--only <service>` mode. Working on one resource is what direct commands are for.

## service deploy

`service deploy` ships one Service: from an artefact (`service build` or the user's own build) or from source through its build profile. It needs no config, runs no migrations, and never turns into a Project deploy (invariant 1). In a config with several Services, a bare `service deploy` is ambiguous; deploying the whole system is `project deploy`.

**Named off-ramp:** `service deploy --db` creates one Branch-scoped database and binds it (`DATABASE_URL` and `DIRECT_URL` as Branch env vars) in the same run. The flag makes the intent explicit. It is the one-command path from nothing to a live URL with a database. It still runs no migrations and never overwrites an existing binding.

## Migrations during project deploy

Pending recorded migrations are part of the plan. Deploy applies the ones whose risk class passes. Destructive or data-loss migrations stop the deploy until the user deals with them directly. No generic confirmation flag gets past that (invariant 9). `service deploy` never runs migrations.

## Promote and rollback

Promote moves Service traffic to a healthy existing Deployment. Rollback moves it back to an earlier one. Neither touches the database, the infrastructure, or other providers. Shipping a full production Project is an explicit production `project deploy`.
...
There is no generic undo. Recovery uses the matching tool: retry the deploy, `service rollback`, `postgres backup restore`, apply a corrected migration, redeploy an earlier revision, or explicit cleanup.
...
# Layer 6 — One contract for humans, agents
...
One name changes meaning and deserves calling out: `dev`. Classic `prisma dev` meant a local Prisma Postgres instance. In Prisma 8, that piece is the postgres emulator (`emulator start postgres`), and `project dev` means a running local copy of the whole project. Decoupling the underlying `@prisma/dev` package from its PPG-only meaning is separate work; the VS Code extension, the ORM, and the Next and Composer test setups all depend on it, and it is off the critical path for consolidation.
...
| `migration plan` / `new` / `check` / `status` / `list` / `show` / `log` / `graph` | `migration …` | Unchanged. The whole group survives as-is: migrate is active, migration is interrogative. graph is kept deliberately: the migration graph is the novel concept, and a first-class command makes it visible. |
| `migrate [--to]` (bare top-level verb) | `db migrate [--to]` | Grammar plus the two-world rule: a bare top-level verb that mutates is the shape we rejected, and migrate acts on the live database the config declares, which is in `db` land. |
| `ref set` / `list` / `delete` (top-level) | `migration ref set` / `list` / `delete` | Refs are migration artefacts on disk; they live with the other interrogatives, never at top level. |
| `db init` | `db init` | Unchanged. The collision that forced a rename disappeared with the `postgres` split; the strict setup policy keeps its name. |
| `db update` | `db update` | Unchanged, for the same reason. `sync` was rejected: it implies a two-way exchange that does not exist. |
...
| Service | Service |
| Module | No platform equivalent, by design: a nestable, reusable grouping of services, databases, and buckets inside the topology. |
...
Two layers sit here on purpose. The platform layer is the deployment target: the primitives and their ownership under Project and Branch. The topology layer is the system the user describes: a composition of modules. Composer owns the mapping between the two. Modules have no platform representation today, and none is needed for consolidation; adding one later is additive and needs no data migration.
...
| `log <entry> [address]` | `service logs` / `project dev` output | Unified verb is `logs`, scoped per Service instead of dotted addresses. |
...
The standalone composer CLI retires when this lands. The Prisma CLI speaks platform terminology (`--branch`, not `--stage`); a target-agnostic composer CLI can return later if real demand appears.

**Current platform CLI (beta) → unified CLI**

| Today | Unified | Why |
| --- | --- | --- |
| `app list-deploys` / `show-deploy` | `service deployment list` / `show` | Nested-noun rule: Deployment has its own identity and lifecycle; compound verbs would be the lone exception to the sub-resource grammar. |
| `app remove`, `project remove`, `app domain remove`, `database connection remove` | `… delete` | Verb rule: `remove` never says whether it means delete, unlink, or detach; `delete` is the one destructive verb. |
| `project env rm` | `project env delete` | Same verb rule. |
| `database …` | `postgres …` | Two-world split: hosted-instance admin is authenticated and addressed by ID; `db` is the connection world. |
| `database remove` | `postgres delete` | Verb rule plus the split. |
| `database restore` | `postgres backup restore` | Nested-noun rule: restore acts on a backup, which owns its own lifecycle. |
| `build logs` (root group) | `service deployment logs` | Root-presence: a build is a phase of a Deployment, not an independent resource. |
| bare multi-app `app deploy` (deploys every configured app) | `project deploy` | Invariant 1: scope from the noun — `service deploy` ships one Service; deploying the declared system is the Project operation. |
| `app promote <deployment>` / `app rollback` | `service promote` / `service rollback` | Unchanged apart from the Service rename. |
...
`service domain retry` and `wait` are conveniences left to later sequencing; leaving them out of the tree is not a removal decision.
...
**J1 — Local-only.** `init`, then `project dev`, then contract and migration work (`contract validate`, `migration plan`, `db migrate`). No login, no Cloud Project, no Branch. Local state stays until reset (`project dev --fresh`). The config can describe just a database and a contract; Services and deployment are not required.
...
**J3 — Direct resources.** `service deploy ./app.tar.gz --project <p> --branch <b>` ships one artifact. `postgres create` returns one database and its connection. No config, no Composer, no migrations. The full direct workflow crosses the worlds on purpose: `postgres create`, then `migration plan`, then `db migrate`, then `db verify`. `service deploy --db` is the one-command path to a live URL with a database. A direct resource can join a Project later with `project adopt`.
...
**J8 — Monorepo.** One config composes several packages. Every Service and Database has a stable key. Nested directories find the Project root. `project dev` starts the topology; `service run` starts one Service and what it depends on. Bindings use names, not pasted URLs. Moving a package does not recreate its cloud resource (invariant 13).
...
- the exact emulator command surface, and the path for decoupling the `@prisma/dev` package from its PPG-only meaning (useful, not on the critical path).

(The former Service-vs-Service question is closed: the noun is Service.)