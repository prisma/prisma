...
Status: PARTIALLY RULED 2026-08-07 — 1–4 recommendations adopted; 5: canonical slug is **prisma/composer** — the bug-report URL uses it AND this slice fixes composer's `package.json#repository` field (currently `prisma/compose`, a mistake); 6: **OVERRIDDEN — populate `where`** on the sites with natural paths (config path: sites 1, 3, 5; entry path: sites 9, 10) since the rendered `Where:` line prints to the user/executing agent — the renderer already handles it, add the corresponding `.where` assertions to the §4 test updates; 7: SUPERSEDED by base-types rules 6–7 (operator, 2026-08-07): **no fallback codes at all** — user-surfaced errors are structured at origin (`LoadError`/`AssembleError`/config-evaluation/stack-write get their own codes at their raise sites; composer core packages import the foundation), and a non-structured error at the boundary is a bug (exit 1, report hint). Additionally the `./control` results reshape onto the shared `Result` discriminator (`ok`), replacing the per-operation `outcome` enums. §3's fallback-map row 41, §5's OperationFailure integration, and the §4 test tables are superseded where they conflict; a design addendum enumerates the deltas. Normative parent: [cli-base-types.md](../../cli-base-types.md) § composer. Target branch: `tml-3174-composer-programmatic-deploy-api` (merges first). Donor: prisma/prisma foundation files. Implementer executes exactly.
...
**Provenance correction (2026-08-07):** the verbatim draft was never committed — the design agent's session ended before it landed, so the pointer above was dangling. The implementer reconstructed the ADR from this document plus the addendum; the orchestrator audited the reconstruction against every item enumerated here and the addendum's §A4 registry/owner deltas and found it complete (including the ruling-1 change: the stage view is described as derivable from the namespace prefix, not a built helper). The normative copy is now the committed file in composer, `docs/design/90-decisions/ADR-0044-errors-are-structural-envelopes-with-dotted-namespace-codes.md` (landed on the PR #205 branch, commit `e5cd6653`).
