...
4. **`UnboundedInt` is a new PostgreSQL-only preset**: unconstrained `numeric` storage, integrality-checked decode to JS `bigint`, decimal-text canonical JSON. Born as the output codec `sumBigInt` needs over `int8` inputs, promoted to a column type because genuinely unbounded integers are a real authoring need (uint256 token amounts). Preset-only — it claims no target-type name, so it cannot collide with the Decimal codec in type position; introspection never infers it.
...
9. **Three slices:** 06 (codecs/presets) ∥ 07 (behaviour-preserving de-hardcoding) → 08 (the breaking operation split with upgrade instructions). 06 and 07 stack on the slice-5 branch; separating 07 from 08 keeps the pure refactor and the behaviour change in different reviews.
