...
- **Outcome:** Columns typed with the new codecs aggregate correctly on both targets: exact-input `SqlAggregateDescriptor` rows are probed against live databases and pinned — PostgreSQL `sum`/`avg` over `int8number` → `pg/numeric@1`, `min`/`max` → `self`; `sum` over `unboundedint` → `pg/unboundedint@1`, `avg` → `pg/numeric@1`, `min`/`max` → `self`; SQLite `sum` over `bigintnumber` → `sqlite/bigint@1` (existing cast-to-text lowering), `min`/`max` → `self`, `avg` per probe — authored in both targets' `aggregates.ts` and pinned by database-backed conformance tests extending the slice-5 aggregate matrices. No existing row changes; no operation vocabulary changes.
