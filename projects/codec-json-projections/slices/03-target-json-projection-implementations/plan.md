...
- **The test environment has degraded past isolation.** Contention began as PostgreSQL connection exhaustion in the four-package parallel run; by D4 the *isolated* `adapter-postgres` package run also fails (13 failures, 12 of them `Client has encountered a connection error`, zero assertions), while per-file runs pass — conformance 65/65, `rls-lifecycle-e2e` 6/6, `native-array-columns` 7/7. From D5 onward the meaningful signal is **per-file, not per-package**, and the slice-DoD full-suite gate at PR-open should be read with that in mind rather than as a clean/dirty verdict.
