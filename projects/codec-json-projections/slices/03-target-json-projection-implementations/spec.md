...
**One codec's wire read moves too.** Pinning `pg/interval@1`'s canonical form necessarily defined what an interval *application value is* — an ISO-8601 duration string — and that redefinition reaches `decode`, not only `encodeJson` / `decodeJson`. A query returning an interval column now yields `'P1D'` where it previously yielded `'{"days":1}'`. This is a query-path change, so the slice's invariant is more precisely: no database-produced **JSON** path changes, and no rendered SQL changes, but one codec's wire decode does. It follows from the operator's decision rather than sitting beside it — a `decode` still returning the old spelling would produce values the codec's own `encodeJson` rejects, so the codec could not round-trip its own output.
