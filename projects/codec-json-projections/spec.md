...
- **Slice 3 is behavior-preserving.** Canonical per-descriptor projections, the PostgreSQL array reference lift, the SQLite retagging mechanism, and the database-backed conformance harness land and are proven against a real database while production renderers still do not invoke `projectJson()`. It advertises nothing and changes no observable output.
