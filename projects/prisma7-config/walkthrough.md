...
- Commit range: `cf2bc1f4b2...cbdef5fd11`
...
3. Share that selection meaning with bootstrap without importing project code. Project detection and seed inspection now agree with runtime discovery, including c12's legacy flat and `index.*` forms.
...
- **Bootstrap matches runtime selection without executing config**: project-state detection and seed inspection use the exported non-executing selector instead of checking only `prisma.config.ts`. Its legacy mirror covers c12 3.3.4's supported flat and `index.*` candidates in the same order, while `package.json` remains the first source for seed commands.
...
- Bootstrap's non-executing legacy selector mirrors the currently pinned c12 3.3.4 discovery shape; parity tests cover its flat/index locations and ordering.
