...
| 3 | `check-enforcement-opt-out` | A per-column, per-kind opt-out (`@noCheck` / `.noCheck()`) for generated checks, and conservative infer emission of it — restoring "pulled schemas verify clean" as the infer default. | ⬜ specced ([spec](./slices/check-enforcement-opt-out/spec.md)) | — |
