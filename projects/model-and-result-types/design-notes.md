...
- `RelationKeys`, `Scalars`, and `With` live in `framework-components` so one symbol serves both families, re-exported from each family's contract types entrypoint.
