...
- The flag is optional at load. A contract without it (a migration snapshot or a file emitted before this change) is hydrated with a value derived from storage: the owning side is nullable when any local foreign-key column is nullable, the non-owning side of a one-to-one is always nullable, and a cross-space relation is nullable. A present flag is checked against storage on load and a disagreement names the columns. The emitter requires the flag on the contracts it consumes. (Settled after code review, 2026-09-10.)
