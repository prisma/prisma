...
- One recursive conditional type in `framework-components/src/execution/shape.ts`, replacing `With`. Distributive over `M`. Validation of names via the constraint on `Spec`, built from `RelationNamesOf<M>` and `keyof Scalars<M>`, so errors land on the offending key.
