...
The namespace segment is present wherever a collision is possible, and dropped where it is not. On Postgres a model in the default schema is `Models.unbound_User` and `typeof models.__unbound__.User`, because `public.User` can sit beside it in the same contract; this matches `db.enums.__unbound__.X` and the contract views. On a target that declares no namespace mechanism, SQLite today, the segment is noise, so the same model is `Models.User` and `typeof models.User`. Mongo contracts carry namespaces and keep the segment. The decision comes from a capability the target declares, never from how many namespaces a particular contract happens to have, so adding a schema later never renames a type.
