...
**Operator-ordered nested storage representation.** `StorageColumn.many` is `false | { elementNullable: boolean }`: when `many.elementNullable` is true, array elements are `T | null`, and the generated `elementNotNull` check does not apply. `StorageColumn.noCheck?: CheckKind[]` remains an independent, explicitly authored enforcement waiver consumed by main's generated-check derivation, diff, introspection, and infer lifecycle. Native `Foo?[]` / `Foo?[]?` lists carry `many: { elementNullable: true }` with no automatic `noCheck`; explicit `Foo[] @noCheck(elementNotNull)` carries only the waiver and keeps non-null element types. An explicit `elementNotNull` waiver on a nullable-element list is rejected because that check is not derivable. Value-object JSONB storage carries neither storage marker nor waiver, while its domain marker is retained.
...
- **Framework contract IR (typing)** — `packages/1-framework/0-foundation/contract`: `ContractField.many` (`domain-types.ts`) becomes `false | { elementNullable: boolean }`, plus validation and canonicalization support for that explicit nested shape.
- **SQL contract IR (storage)** — uses `StorageColumn.many: false | { elementNullable: boolean }` as semantic/type metadata and reuses main's existing `StorageColumn.noCheck` representation only for explicit enforcement waivers. Nullable native-list elements carry the marker without an automatic waiver. The marker is not projected into schema IR or compared for migration drift.
...
- **Postgres generated-check lifecycle** — generated-check derivation reads `StorageColumn.many.elementNullable` and does not produce an `elementNotNull` candidate when it is true; the existing `StorageColumn.noCheck` filter remains for explicit waivers on semantically non-null lists.
...
- **Intentional wire migration.** Cardinality is explicit: non-list fields emit `many: false`, and list fields emit `many: { elementNullable: false | true }`. Existing strict-list DDL remains unchanged even though contract JSON and hashes adopt the new nested representation.
...
1. **Operator-ordered nested IR.** `ContractField.many` and `StorageColumn.many` are `false | { elementNullable: boolean }`, coupling cardinality to element semantics so invalid parallel states are unrepresentable. Native nullable-element lists carry `many: { elementNullable: true }` and directly suppress `elementNotNull` candidate generation; they do not carry an automatic `noCheck`. Explicit `Foo[] @noCheck(elementNotNull)` retains `many: { elementNullable: false }`, and the same waiver on `Foo?[]` is rejected as inapplicable. Value-object lists remain JSONB with neither storage marker nor waiver, while retaining the domain marker.
...
3. **TS authoring.** In scope and DoD-gated. SQL builder API: `.many({ elementsNullable: true })`; omitted or false means non-null elements and emits `many: { elementNullable: false }`. The option is structurally coupled to list cardinality.
