...
If the project also matches the extension-author role, install the `prisma-next-extension-upgrade` skill (`pnpm dlx skills add prisma/prisma-next/skills --skill prisma-next-extension-upgrade -y`) and run **this** flow first, then that one in the same session. If detection is ambiguous, ask the user.
