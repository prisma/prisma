...
**Skills under test:** `prisma-8-build`, `prisma-8-feedback`.
...
- [ ] Skill matcher fires on `prisma-8-build` (description contains "next plugin", "next.js plugin", "withPrismaNext").
...
- [ ] Agent asks the user if they want to file a feature request and, if yes, routes to the `prisma-8-feedback` skill (does NOT open an issue without explicit user confirmation).
...
- [ ] The user is offered the `prisma-8-feedback` route for filing a request.
