...
**Example app:** A fresh directory scaffolded with `pnpm dlx prisma-next init` (run inside the new directory — `init` operates in `cwd` and does not take a project-name argument), or `examples/prisma-8-demo` with the User model deleted.
