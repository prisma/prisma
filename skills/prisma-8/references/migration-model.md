...
| `db sign` | Advances `db` after a successful signature (override the name with `--advance-ref <name>`); an existing ref is overwritten and the previous hash is reported. `--db` does **not** suppress it, unlike init/update: sign never mutates the schema, and adoption is normally done via `--db` |
