...
Import `acc` and expression helpers (`fn`) from `@prisma/orm-mongo/query-builder` when building computed pipeline stages.
...
10. **Expecting `.delete()` / `.update(...)` to affect every matching document.** They affect one document (the first match) and return it. Use `.deleteAll()` / `.updateAll(...)` or the `*AndCount` terminals for many-document writes.
