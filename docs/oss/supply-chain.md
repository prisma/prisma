...
## PR measurement, scanning, and previews

[Bundle Size](../../.github/workflows/bundle-size.yml), [PR Code Security](../../.github/workflows/pr-code-security.yml), and [Preview Release](../../.github/workflows/preview-publish.yml) use `contents: read`, do not pass secrets to PR code, and disable checkout credential persistence. No privileged reporting job consumes their artifacts or caches.

- Bundle Size runs the Size Limit CLI for both PRs and merge groups. Configured limits still fail the check; measurements are available in the `size-limit-results` artifact. Automatic PR comments and base-versus-head comparisons are no longer generated.
- Secret Detection invokes the MIT-licensed Gitleaks CLI v8.24.3 directly, with the release archive's SHA-256 pinned in the workflow. Unlike the Gitleaks GitHub Action's organization license gate, the CLI needs neither a license key nor a GitHub API token. It scans commits in `base.sha..head.sha` (including secrets introduced and later removed within the PR), exits nonzero on findings, and uploads fully redacted SARIF. Forks are scanned too. This is a PR-commit scan, not a replacement for repository-wide historical scanning; repository configuration and ignore files can affect detection and require review.
- Semgrep retains its error-severity check and SARIF artifact, but no longer runs Reviewdog with a write token. Dependabot PRs now scan too. Rule retrieval via `--config auto` still requires network access; the existing container version and rule policy are unchanged.
- Preview Release still builds and publishes packages to pkg.pr.new on non-inert PRs and pushes to main. The pinned CLI authenticates through its service's GitHub App/run validation, not a supplied `GITHUB_TOKEN`. Comments are disabled; install URLs remain in logs and `preview-packages`. Package paths are validated and passed as shell-array arguments, never interpolated as shell source. Packing still executes PR-controlled code in the read-only job. Preview packages, URLs, and reports are untrusted output, not approved releases.

Artifacts expire after seven days. There is deliberately no write-token reporting phase: moving reporting to another PR-controlled job would not establish a trust boundary. Reintroducing comments requires a separately reviewed, trusted mechanism that treats PR data as data and never executes its code, artifacts, or cached outputs.

## Remaining isolation requirements (administrator-owned)

These workflow changes remove credential exposure in the checked-in execution paths; **they do not isolate same-repository PRs that can edit workflow YAML**. Such a PR can request other available repository/organization secrets or change requested token permissions. A workflow test or code-owner review can detect changes but cannot prevent secret access before merge. Repository settings and available secret-name lists were not accessible during this audit (403), so their protection has not been verified.

Administrators must verify that production credentials are unavailable to untrusted workflows: move them out of broadly accessible repository/organization scope into protected environments with trusted deployment branches, required reviewers, and appropriate bypass/self-review restrictions (or a separately controlled release repository). Restrict external OIDC trust to the intended repository, workflow, ref/environment. Review Actions permissions, fork approval policy, GitHub App permissions, workflow ownership, and cache access; never restore untrusted PR build outputs into privileged release/deploy execution. Review artifacts as untrusted data, not commands or authoritative security attestations.

Release/deploy semantics are unchanged by this hardening. [Publish to npm](../../.github/workflows/publish.yml) permits a **non-main manual dry run with `contents: write`, OIDC permission, and persisted checkout credentials**. Dry-run flags prevent intended publishing, not malicious build code using those credentials. [Deploy telemetry-backend](../../.github/workflows/deploy-telemetry-backend.yml) permits manual runs from any branch and later injects production credentials on the same runner that built that branch. Until separately approved isolation changes land, dispatch either workflow only on a reviewed, trusted revision; do not run a PR branch to validate it with production/write credentials. Delaying secret injection until a later step does not protect it from earlier code on the same runner.
