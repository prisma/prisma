...
## The integer representation types

64-bit integers travel through database-produced JSON as decimal text — with one deliberate exception, described here. Two opt-in target-scoped types give a column a different integer representation, each backed by a codec beside its siblings in the target's `codecs.ts`:

| Type | Targets | Codec | Storage | Application value | Canonical JSON |
| --- | --- | --- | --- | --- | --- |
...
Bare `BigInt` keeps the lossless `bigint` codec (`pg/int8@1` / `sqlite/bigint@1`); a column uses bare `BigIntNumber` or `UnboundedInt` to opt into an alternative representation. SQLite declares no `UnboundedInt` because it has no lossless unbounded integer storage, and type availability is the target's declaration.
...
**Target-scoped authoring, no reverse target-type claim.** All three descriptors declare `targetTypes: []`, so `int8`, `numeric`, and SQLite's `integer` in type position keep their current codecs and introspection stays unambiguous. The active target contributes `BigIntNumber` and, on PostgreSQL, `UnboundedInt` as ordinary authoring type constructors. PSL uses the bare type names. TypeScript either registers the composed `type.BigIntNumber()` / `type.UnboundedInt()` results in the contract's `types` map and passes those same instances to `field.namedType(...)`, or uses the exported direct helpers through `field.column(pgInt8NumberColumn())`, `field.column(pgUnboundedIntColumn())`, or `field.column(sqliteBigintNumberColumn())`. Authoring availability does not make a reverse-introspection claim.
