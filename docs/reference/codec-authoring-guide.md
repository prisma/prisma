...
The guarantee reaches values the query **computes** as well as values it stores. An aggregate has no column codec to be canonical against, so its target declares one — see [Aggregate result codecs](#aggregate-result-codecs) — and the declared codec is what the value enters JSON under and is read back through. A count inside an `.include()` arrives as decimal text and reads back as the `bigint` it is, rather than as a JSON number that cannot hold it. The one case left outside is an aggregate no target declares an overload for: it carries no codec, and its value arrives as the driver hands it over.
...
An aggregate's result enters a JSON envelope wherever it is an include reducer, and it goes in under the codec resolved here — which is why the canonical-JSON rules above apply to aggregates too. A count past 2^53 read as a JSON number is the same defect as a `numeric` read as one.
