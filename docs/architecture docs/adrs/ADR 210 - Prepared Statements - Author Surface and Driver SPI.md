...
A SQL-builder callback ends with `.build()`, returning a plan; raw SQL callbacks also return plans. `Row` is derived from that plan's row type. An ORM facade callback ends with `.prepared.all()` or `.prepared.first()`, returning a description rather than executing an ordinary terminal. ORM predicates preserve non-nullable scalar placeholders in shorthand, callback, relation/include and terminal filters, including fixed lists of individual operands. Structured ORM comparisons reject nullable prepared references before execution, using declaration nullability retained on the frozen `PreparedParamRef` AST node. Shared ORM predicate binding enforces this boundary for nested expressions and structured interoperability inputs, without changing SQL-builder equality. Raw SQL remains opaque, including interpolation payloads: it is neither rewritten nor rejected based on nullable parameters. Literal-null ORM filters retain their null checks. ORM aggregate/mutation preparation, dynamic parameter lists and expression-valued ORM pagination are not part of this row-terminal surface.
