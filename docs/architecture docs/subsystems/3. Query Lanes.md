...
- **Chained Methods**: `where()`, `orderBy()`, `limit()`, `offset()`, `select()`
...
- **`Collection` chaining API** — `.where().select().include().orderBy().limit().offset().all().first()` with immutable method chaining
...
- Projection features
  - `.distinct()` requires no capability — it lowers to a `ROW_NUMBER` dedup, portable across every target this system adapts to.
  - `.distinctOn()` requires `postgres.distinctOn` (Postgres-only; the sql-builder and ORM lanes both gate on this key).
