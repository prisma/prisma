...
Implicit default logic lives in [`computeRefAdvancementName`](../../../packages/1-framework/3-tooling/cli/src/control-api/operations/ref-advancement.ts): omitting both `--advance-ref` and `--db` yields `db`; supplying `--db <non-default-url>` without `--advance-ref` yields no advancement. `db sign` does not go through it: the sign command picks `--advance-ref <name>` if given, else `db`.
