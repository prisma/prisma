...
**Scope — snapshot store entry drift.** `PN-MIG-CHECK-005` catches drift between a migration's `metadata.to` and its snapshot store entry (the file at `migrations/snapshots/<hex>/contract.json` should itself carry `storage.storageHash === <hex>`). It does **not** detect cross-migration consistency (e.g. migration A's destination store entry differs from the contract migration B's predecessor hash should resolve to). Proving that recorded snapshots produce the same physical schema would require executing migrations against a database, which the system deliberately never does offline (no shadow database exists). End-state divergence surfaces at apply time instead: the runner's destination-hash post-check and `db verify` both report a database that did not land on the promised contract.
