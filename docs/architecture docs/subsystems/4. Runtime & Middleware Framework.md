...
For a query, the runtime runs `beforeCompile` where the family provides it, lowers the plan, runs `beforeQuery`, encodes parameters, runs `interceptQuery`, and then either consumes intercepted rows or calls the driver's `query` terminal. Driver rows pass through `onRow` and `afterQuery`. For an execute, the same shared preparation runs with `beforeExecute` and `interceptExecute`, then either returns intercepted statistics or calls the driver's `execute` terminal and runs `afterExecute`. `beforeCompile` is shared; all later hooks are operation-specific. The matching before hook runs before encoding so a parameter mutator can affect the wire values, while interceptors observe the encoded execution plan.
...
- `lower(plan: SqlQueryPlan): SqlExecutionPlan` — runs the SQL lowering stages. The selected `beforeQuery` or `beforeExecute` chain runs before parameter encoding.
- `runDriver(exec: SqlExecutionPlan): AsyncIterable<Record<string, unknown>>` — the default row driver invocation. Statistics use the matching `runExecute(exec)` terminal and return `SqlStatementStats`.
...
- `lower(plan: MongoQueryPlan): MongoExecutionPlan` — runs the adapter's structural lowering and parameter resolution around the selected before hook.
- `runDriver(exec: MongoExecutionPlan): AsyncIterable<Record<string, unknown>>` — drives `driver.execute(exec.command)` for row results; Mongo statistics map update `modifiedCount` or delete `deletedCount` in `runExecute(exec)`.
...
sequenceDiagram
  ...
  participant Lower as Lower + encode
  ...
  Runtime->>Lower: selected beforeQuery/beforeExecute, then encode
  Lower-->>Runtime: encoded execution plan
  alt query
...
The runtime executes Plans through the small set of well-defined stages defined by `RuntimeCore`. SQL's AST-backed lanes participate in shared `beforeCompile`; both families lower into their `*ExecutionPlan`; each selected operation then runs its own before, interception, driver, row/statistics, and after stages. This structure keeps behavior explicit, enables early failure with actionable errors, and prevents statistics from being inferred from rows.

```text
TPlan (e.g. SqlQueryPlan with AST + meta, no SQL yet)
  └─▶ beforeCompile                 // SQL shared AST rewrite; default: identity for Mongo
       └─ lower to draft            // family-specific; params are still user-domain values
           └─▶ beforeQuery or beforeExecute
               └─ encode parameters
                   └─▶ interceptQuery or interceptExecute
