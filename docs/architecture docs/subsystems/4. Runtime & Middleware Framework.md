...
flowchart TB
  ...
  SR -.extends.-> Core
  MR -.extends.-> Core
  SR -- lower --> LowerS[lowerToDraft]
  SR -- encode --> EncodeS[encodeDraftParams]
...
- `lower(plan: SqlQueryPlan, ctx: SqlCodecCallContext): SqlExecutionPlan` — runs the SQL lowering stages. The selected `beforeQuery` or `beforeExecute` chain runs before parameter encoding.
