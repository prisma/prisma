...
sequenceDiagram
  ...
  participant Compile as SQL beforeCompile (AST-backed plans only)
  participant Lower as Structural lower
  participant Encode as Encode parameters
  participant MW as Middleware
  participant Driver as Driver

  App->>Runtime: query(plan) or execute(plan)
  alt AST-backed SQL plan
    Runtime->>Compile: plan
    Compile-->>Runtime: rewritten plan
  else already-lowered SQL plan
    Note over Runtime: bypass beforeCompile
  end
  Runtime->>Lower: structural lower
  Lower-->>Runtime: user-domain draft
  Runtime->>MW: selected beforeQuery/beforeExecute
  MW-->>Runtime: mutated draft
...
TPlan
  └─▶ family compile step (SQL beforeCompile for AST-backed plans; Mongo identity)
...
4. Execute
   - The caller selects `query(plan)` for rows or `execute(plan)` for statistics. For AST-backed SQL plans, the runtime runs `beforeCompile`; raw SQL plans bypass it. The selected before hook, encoding, matching interceptor, and either the matching driver terminal or intercepted result then run as documented. Query rows pass through `onRow` and `afterQuery`; execute statistics pass through `afterExecute`.
5. Release
...
const posts = await db.transaction(async (tx) => {
  const materializedPosts = await tx.orm.posts.all()
