...
- **Naming and equivalence.** A policy is either **managed** or **exact-named**. Managed (the default): the authored name is a prefix, the target appends an 8-hex SHA-256 suffix over the canonical content tuple, and equivalence is wire-name identity — the verifier never compares predicate bodies ([ADR 234](../adrs/ADR%20234%20-%20Content-addressed%20wire%20names%20for%20Postgres-normalized%20objects.md)). Exact (`@@map("physical name")` on the policy block): the name is adopted verbatim — no prefix, no hash, and no managed-prefix cap (the name is still a physical identifier, so the target's own limits apply; Postgres truncates identifiers to 63 bytes) — and equivalence becomes content comparison (structured attributes strict, `using`/`withCheck` byte-for-byte), reliable precisely when the body text is a Postgres reprint; hand-authoring a body under `@@map` warns (`PN_EXACT_NAME_BODY_COMPARISON`, shared per-build batch with index `map:`).
