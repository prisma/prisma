...
- **Signing** — `db sign`. Verifies a live DB satisfies a contract, then writes the contract hash into the marker and advances the `db` ref to it (`--advance-ref <name>` overrides; `--db` does not suppress it). The adoption path for an already-matching database. No structural changes.
...
- **`db sign [<contract>]`** *(explicit form: `db sign --contract <contract>`)* — verify the live DB satisfies a contract, then write the contract hash into the marker and advance the `db` ref to the signed hash (`--advance-ref <name>` overrides; unlike `db init` / `db update`, `--db` does not suppress it). **Refuses if it doesn't satisfy.** No structural mutation. The adoption path for an already-matching DB. Without an argument, defaults to the current `contract.json`. The argument names *the thing being signed* — distinct from `--to` (movement) used by `migrate` and `db update`.
