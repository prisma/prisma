...
- **MongoDB rejects previously ignored attributes.** Remove unsupported `@default`, `@updatedAt`, and `@db.*` attributes from MongoDB schemas only. They never produced defaults or timestamps in the MongoDB contract; these remain application responsibilities. Unknown model and field attributes now fail emission, and `@id`/`@unique` reject arguments. Direct callers of `interpretPslDocumentToMongoContract()` must also supply its new `controlMutationDefaults` registry; normal provider users need no configuration change. See the [app upgrade recipe](https://github.com/prisma/orm/blob/v8.0.0-rc.9/skills/prisma-8/upgrading/app/upgrades/8.0.0-rc.8-to-8.0.0-rc.9/) for schema migration. ([#30160](https://github.com/prisma/orm/pull/30160))
...
- **Extension attribute APIs use explicit contexts and parsed block attributes.** Replace `InterpretCtx` with the appropriate attribute context, name context type arguments explicitly, and add the required `PslExtensionBlock.attributes` record to synthetic blocks. Malformed native-enum `@@map` arguments now report `PSL_INVALID_ATTRIBUTE_SYNTAX` instead of `PSL_NATIVE_ENUM_INVALID_MAP`. See the [extension upgrade recipe](https://github.com/prisma/orm/blob/v8.0.0-rc.9/skills/prisma-8/upgrading/extension/upgrades/8.0.0-rc.8-to-8.0.0-rc.9/) for the complete context and argument-parser migration. ([#30162](https://github.com/prisma/orm/pull/30162))
...
- **Contributed model-attribute specs must be factories.** Change `AuthoringModelAttributeDescriptor.spec` from a spec value to a function returning the spec. Hand-built assembled authoring contributions must also include `attributeSpecs: { model: {}, field: {} }`. ([#30154](https://github.com/prisma/orm/pull/30154))
...
- Extension and tooling authors can register and enumerate contextual model/field attribute specs, inspect built-in SQL and MongoDB attribute namespaces, and declare block attributes with `blockAttribute()`. ([#30154](https://github.com/prisma/orm/pull/30154), [#30159](https://github.com/prisma/orm/pull/30159), [#30160](https://github.com/prisma/orm/pull/30160), [#30162](https://github.com/prisma/orm/pull/30162))
- Attribute-spec authors gain a quoted-JSON-object combinator and literal-preserving `str(value)`/`num(value)` combinators. ([#29833](https://github.com/prisma/orm/pull/29833))
...
- Attribute-alternative errors identify accepted function names, such as `now()` and `uuid()`, instead of repeating “function call.” ([#30224](https://github.com/prisma/orm/pull/30224))
