...
- `ORDER BY` / `DISTINCT ON` on an enum column now sort by declaration order behind a derived table (`distinct()`, `groupBy()`), instead of silently falling back to lexical text order. ([#30099](https://github.com/prisma/prisma/pull/30099))
