...
The human reviewer + the CI gate (`pnpm check:upgrade-coverage`) both check this shape, but the gate is **necessary-but-not-sufficient**, and what it checks is mechanical rather than semantic. Its `per-pr-declaration` rule asserts that *this PR's own diff touches* `<transition>/instructions.md` and that the file's frontmatter carries a parseable `changes:` array. It does not ask whether the diff is *covered* by what is already recorded — so an instructions file that already exists with valid entries does not satisfy the gate, and every PR with a substrate diff must re-touch it. Conversely, touching the file and declaring `changes: []` passes green while a real breaking change ships undocumented. That second gap is load-bearing for the reviewer: **the human reviewer must verify that every substrate diff in the PR has a corresponding entry**, and catches the semantic case (entry exists but its prose / scripts don't match the framework change).
