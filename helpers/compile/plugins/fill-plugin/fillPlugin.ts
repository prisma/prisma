...
const loader = (cache: LoadCache) => (module: string) => {
  ...
  const modulePath = path.dirname(require.resolve(modulePkg))
 ...
 * @param $ to be scanned
 */
function setCodeBannerAndFooter($: Fillers['$'], options: esbuild.BuildOptions) {
  if ($?.import !== undefined) {
 ...
 * @param fillers override default fillers
 * @returns
 */
export const fillPlugin = (fillers: Fillers): esbuild.Plugin => ({
  name: 'fillPlugin',
  setup(build) {
    const { $, ...injectionFillers } = fillers

    // our first step is to update options with basic injections
    setCodeBannerAndFooter($, build.initialOptions)
    setInjectionsAndDefinitions(injectionFillers, build.initialOptions)

    // allows us to change the path of a filtered import by another
    build.onResolve({ filter: createImportFilter(injectionFillers) }, (args) => {
      return onResolve(injectionFillers, args)
    })

    // if no path was provided it defers to virtual nsp `fill-plugin`
    build.onLoad({ filter: /.*/, namespace: 'fill-plugin' }, (args) => {
      return onLoad(injectionFillers, args)
    })
  },
})
