  ...
  setup(build) {
    ...
    build.onStart(async () => {
      ...
      if (options.bundle === true && options.entryPoints?.[0].endsWith('.ts')) {
        ...
        const entryPoint = options.entryPoints?.[0].slice(0, -3) // drop ".ts"
        const typeOutPath = `${typeOutDir}/${entryPoint.slice(4)}` // drop "src/"
        const bundlePath = options.outfile!.slice(0, -3) // drop ".ts"
