 ...
 */
export function tscPlugin({ emitTypes, overrideDev }: { emitTypes?: boolean, overrideDev?: true }): esbuild.Plugin {
  return {
    name: 'tscPlugin',
    setup(build) {
      ...
      build.onStart(async () => {
        ...
        // or if we are explicitly told to override the dev mode.
        if (overrideDev || (process.env.WATCH !== 'true' && process.env.DEV !== 'true')) {
