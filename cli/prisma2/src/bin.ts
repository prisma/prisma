...
if (process.argv.length > 2) {
  ...
  if (args && args['--schema']) {
    ...
    if (fs.existsSync(args['--schema']) && fs.existsSync(dotenvFilepath)) {
      ...
      console.log(chalk.dim('.env loaded from provided --schema directory'))
    } else {
    ...
    console.log(chalk.dim('.env loaded from current directory'))
    ...
    console.log(chalk.dim('.env loaded from ./prisma/.env'))
