...
- Build a narrow JS-owned query / Rust-owned IR proof point for one read-only cache-hit shape: pass one JS request reference, walk it once to parameterize and compute structural identity, check the plan cache before building owned Rust request maps, and return an already cached JS plan object on hits. This must replace multiple current phases at once; prior `serde_wasm_bindgen`, `js_sys` cache-key-only, and TypeScript fused-writer spikes were too shallow or slower.
  ...
  - Success criterion for the first proof point: one read-only cache-hit shape should avoid `JSON.stringify()`, `JsonBody` / `serde_json::Value` materialization, `ArgumentValue` ownership for unchanged input values, and plan serialization back through Wasm memory. Compile misses can initially fall back to the current Rust-owned SQL builder; JS-owned SQL/template handles should be measured as a second-stage project rather than bundled into the first spike.
...
- Accepted experiment: omit empty JSON protocol `arguments`.
  - Timestamp: 2026-06-07T17:01:58Z.
  - Change:
    - `serializeFieldSelection()` now omits the `arguments` property when serialized arguments are empty.
    - Non-empty `arguments` still appear before `selection` in object insertion order to avoid unnecessary cache-key and snapshot churn.
    - Raw-tagged argument payloads still force the `arguments` property to preserve raw query behavior.
    - Updated inline JSON protocol snapshots to reflect omitted empty `arguments`.
  - Rationale:
    - `JsonFieldSelection.arguments` is already optional in `packages/json-protocol`, Rust `FieldQuery.arguments` is `Option<IndexMap<...>>`, and the Rust JSON protocol adapter treats `None` as an empty argument list.
    - Empty `arguments: {}` objects were emitted for every selection-only node. They increased serializer allocation, parameterization traversal, cache-key JSON, and Wasm request size without semantic value.
  - Timing:
    - First patched generated-client run: `findUnique` 4.50 us/op, nested blog-page 19.17 us/op, nested heap delta 299.9 KiB.
    - Same-session reverted control: `findUnique` 4.64 us/op, nested blog-page 19.37 us/op, nested heap delta 312.8 KiB.
    - Reapplied current-form run after preserving non-empty argument order: `findUnique` 4.73 us/op, nested blog-page 19.51 us/op, nested heap delta 310.8 KiB.
    - Workerd high-iteration smoke after build: worker-internal generated `findUnique` 6.03 us/op and nested blog-page 17.95 us/op. This is mixed versus the previous accepted 5.96 / 18.30 us/op row, but nested moved in the right direction.
    - Current-form `caching.bench.ts`:
      - compile rows: `findUnique` 1,696 ops/sec, filtered `findMany` 1,347 ops/sec, blog-page 362 ops/sec.
      - parameterization rows: `findUnique` 1,727,922 ops/sec, filtered `findMany` 748,614 ops/sec, `findMany in filter` 817,448 ops/sec, blog-page 692,159 ops/sec.
      - cache-hit key rows: `findUnique` 969,166 ops/sec, filtered `findMany` 499,926 ops/sec, `findMany in filter` 571,666 ops/sec, blog-page 362,443 ops/sec.
  - Verification:
    - `pnpm exec prettier --check packages/client/src/runtime/core/jsonProtocol/serializeJsonQuery.ts`
    - `pnpm exec eslint packages/client/src/runtime/core/jsonProtocol/serializeJsonQuery.ts`
    - `pnpm exec jest --silent --runTestsByPath src/runtime/core/jsonProtocol/serializeJsonQuery.test.ts --runInBand -u` from `packages/client`
    - `pnpm --filter @prisma/client test -- --runTestsByPath packages/client/src/runtime/core/jsonProtocol/serializeJsonQuery.test.ts packages/client/src/runtime/core/jsonProtocol/getBatchId.test.ts --runInBand`
    - `pnpm --filter @prisma/client build`
    - `LOCAL_QC_BUILD_DIRECTORY=/home/aqrln.guest/prisma-engines/query-compiler/query-compiler-wasm/pkg WORKERD_GENERATED_FIND_UNIQUE_ITERATIONS=100000 WORKERD_GENERATED_BLOG_PAGE_ITERATIONS=20000 pnpm exec node --expose-gc --import tsx packages/client/src/__tests__/benchmarks/query-performance/workerd-query-compiler-memory.ts`
    - `LOCAL_QC_BUILD_DIRECTORY=/home/aqrln.guest/prisma-engines/query-compiler/query-compiler-wasm/pkg pnpm exec tsx packages/client/src/__tests__/benchmarks/query-performance/caching.bench.ts`
...
- Accepted measurement: generated client serializer timing row.
  - Timestamp: 2026-06-07T17:32:55Z.
  - Change:
    - Added `generated client serialize ...` rows to `packages/client/src/__tests__/benchmarks/query-performance/client-engine-cache-timing.ts`.
    - The row constructs generated-style args and calls `serializeJsonQuery()` with the benchmark runtime data model, but does not create PrismaPromises, enter DataLoader, compute cache keys, or execute the engine.
  - Rationale:
    - The generated-client profile still showed serializer functions prominently, but the full generated row mixed serializer, request handling, cache-key, executor, and adapter work.
    - The new promise-construction row showed proxy/PrismaPromise construction was tiny, so the next split needed to isolate serializer cost.
  - Timing:
    - `generated client serialize findUnique / warmed cache`: 0.32 us/op.
    - `generated client serialize blog page / nested rows warmed cache`: 4.17 us/op.
    - Same run promise construction rows: 0.26 / 0.49 us/op.
    - Same run full generated rows: 4.94 / 18.24 us/op.
  - Verification:
    - `pnpm exec prettier --check packages/client/src/__tests__/benchmarks/query-performance/client-engine-cache-timing.ts`
    - `pnpm exec eslint packages/client/src/__tests__/benchmarks/query-performance/client-engine-cache-timing.ts`
    - `LOCAL_QC_BUILD_DIRECTORY=/home/aqrln.guest/prisma-engines/query-compiler/query-compiler-wasm/pkg CLIENT_ENGINE_CACHE_TIMING_FILTER='generated client' CLIENT_ENGINE_CACHE_TIMING_ITERATIONS=500000 pnpm exec node --expose-gc --import tsx packages/client/src/__tests__/benchmarks/query-performance/client-engine-cache-timing.ts`
...
- Accepted measurement: static protocol query lower bound.
  - Timestamp: 2026-06-07T20:16:03Z.
  - Change:
    - Added `internal request precomputed static protocol lazy descriptor ...` rows to `client-engine-cache-timing.ts`.
    - This row reuses one protocol query object while still extracting descriptor placeholder values from generated-style args and going through `client._request()` / `RequestHandler` / `ClientEngine.request()`.
  - Rationale:
    - The dynamic precomputed-protocol row still allocated the equivalent protocol query on every call. This row measures whether protocol tree construction is a large part of the remaining gap to the direct cached-wrapper lower bound.
  - Measurement:
    - `LOCAL_QC_BUILD_DIRECTORY=/home/aqrln.guest/prisma-engines/query-compiler/query-compiler-wasm/pkg CLIENT_ENGINE_CACHE_TIMING_FILTER='internal request precomputed' CLIENT_ENGINE_CACHE_TIMING_ITERATIONS=100000 pnpm exec node --expose-gc --import tsx packages/client/src/__tests__/benchmarks/query-performance/client-engine-cache-timing.ts`
      - `internal request precomputed lazy descriptor findUnique / warmed cache`: 4.12 us/op.
      - `internal request precomputed lazy descriptor blog page / nested rows warmed cache`: 16.15 us/op.
      - `internal request precomputed protocol lazy descriptor findUnique / warmed cache`: 3.15 us/op.
      - `internal request precomputed protocol lazy descriptor blog page / nested rows warmed cache`: 12.23 us/op.
      - `internal request precomputed static protocol lazy descriptor findUnique / warmed cache`: 3.11 us/op.
      - `internal request precomputed static protocol lazy descriptor blog page / nested rows warmed cache`: 11.82 us/op.
  - Decision:
...
- Accepted optimization: RequestHandler two-request batch loader fast path.
  - Timestamp: 2026-06-07T23:54:22+02:00.
  - Change:
    - Updated `packages/client/src/runtime/RequestHandler.ts` so `batchLoader` handles exactly two requests with direct array construction and manual result mapping.
    - The generic path remains unchanged for larger batches.
  - Rationale:
    - The same common `Promise.all([findUnique, findUnique])` path reaches `RequestHandler.batchLoader()` with exactly two requests. The previous code paid `map` / `every` / `map` / `some` setup cost and a final `results.map()` even for this fixed-size hot path.
  - Verification:
    - `pnpm exec prettier --write packages/client/src/runtime/RequestHandler.ts`
    - `pnpm --filter @prisma/client test RequestHandler.test.ts --runInBand`
    - `pnpm exec eslint packages/client/src/runtime/RequestHandler.ts`
      - Passed with the existing `RequestHandler.ts` unsafe-argument warnings.
    - `pnpm --filter @prisma/client build`
  - Node measurement:
    - Command:
      - `LOCAL_QC_BUILD_DIRECTORY=/home/aqrln.guest/prisma-engines/query-compiler/query-compiler-wasm/pkg CLIENT_ENGINE_CACHE_TIMING_FILTER='batched findUnique' CLIENT_ENGINE_CACHE_TIMING_ITERATIONS=100000 pnpm exec node --expose-gc --import tsx packages/client/src/__tests__/benchmarks/query-performance/client-engine-cache-timing.ts`
    - Rows:
      - `generated client promise construction batched findUnique / warmed cache`: 2.89 us/op.
      - `generated client batched findUnique / warmed cache`: 12.87 us/op.
      - `generated client engine precomputed fast path batched findUnique / warmed cache`: 5.48 us/op, `queryRaw=200000`.
      - `generated client request precomputed fast path batched findUnique / warmed cache`: 7.66 us/op, `queryRaw=100000`, `precomputedBatchHits=200000`.
    - Previous accepted DataLoader two-job fast-path focused row was 8.08 us/op.
  - Workerd measurement:
    - Command:
      - `LOCAL_QC_BUILD_DIRECTORY=/home/aqrln.guest/prisma-engines/query-compiler/query-compiler-wasm/pkg WORKERD_CLIENT_CACHE_KEY_ITERATIONS=10 WORKERD_DESCRIPTOR_ITERATIONS=10 WORKERD_PRECOMPUTED_ITERATIONS=10 WORKERD_GENERATED_FIND_UNIQUE_ITERATIONS=10000 WORKERD_GENERATED_BLOG_PAGE_ITERATIONS=1000 pnpm exec node --expose-gc --import tsx packages/client/src/__tests__/benchmarks/query-performance/workerd-query-compiler-memory.ts`
    - `generated client batched findUnique warmed cache`: worker loop 15.00 us/op, host dispatch 18.29 us/op.
    - `generated client request precomputed fast path batched findUnique warmed cache`: worker loop 9.20 us/op, host dispatch 11.13 us/op, `queryRaw 10000`, `precomputed batch: hits 20000`.
    - Previous accepted DataLoader two-job fast-path Workerd row was 9.70 us/op worker loop / 11.63 us/op host dispatch.
  - Decision:
...
- Explore Rust-owned IR only, with JS-owned input and plan/cache objects.
  ...
  - First proof point: one read-only cache-hit shape where the Wasm entrypoint accepts a JS request reference, computes the structural cache identity while walking JS data, returns a cached JS plan object on hit, and never materializes `JsonBody` / `serde_json::Value` / owned `ArgumentValue` for unchanged input values.
