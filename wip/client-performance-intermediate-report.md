...
2. Move raw-nested execution toward generated/static writer schedules. The benchmark-only static-wave writer reaches the assembler lower bound, while runtime-derived generic schedules, local wrapper/leaf shortcuts, row-mapper unrolling, and the serializable compact `N` writer-program node keep regressing or only moving a few percent. A fresh 2026-06-10 300k run measured generated exact-helper blog-page at 10.21 us/op, direct plan at 6.55, compact node at 6.27, and static-wave writer at 3.82. The next serious proof should flatten recursive raw-nested reads into a declarative row-source/write graph, then compile that graph into CSP-safe, width-specialized waves that initialize relation slots and write final owner objects directly. A runtime-only prototype can compile today's `RawNestedReadQuery` into that internal graph first; if it wins, replace the internal serialized raw-nested format outright instead of carrying old/new readers. Wire Rust query-compiler emission only if an internal/benchmark path moves Node and Workerd raw-result plus generated blog-page gates clearly toward the static-wave lower bound.
