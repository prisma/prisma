...
- The latest compact-only internal-format cleanups removed legacy object-shaped `templateSql`, tagged fragments, legacy query-plan PrismaValue placeholder/generator readers, legacy validation rule/error readers, and legacy binding/join support structures from the runtime/types/tests. This is smaller than the earlier memory wins, but it keeps internal hot paths version-lockstep-only. Close direct-plan timing A/Bs over 300k nested blog-page iterations measured 12.62 us/op control vs 10.93 us/op patched for `templateSql`, then 9.05 us/op control vs 8.52 us/op patched for PrismaValue after preserving the monomorphic local-object guard shape; the validation cleanup is not on this cached read path, and the direct-plan smoke stayed in the recent band at 8.54 us/op. The tuned binding/join cleanup measured 8.34 vs 8.36 us/op in a 1M patched/control pair and 8.31 us/op on a 300k confirmation run after direct-indexing hot interpreter paths. The cache-memory probe stayed in band at 4.47 MiB for 1,000 node-warm blog-page plans and 4.69-4.70 MiB for the parameterized row.
