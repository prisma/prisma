...
2. Move raw-nested execution toward generated/static writer schedules. The benchmark-only static-wave writer reaches the assembler lower bound, while runtime-derived generic schedules, local wrapper/leaf shortcuts, row-mapper unrolling, and the serializable compact `N` writer-program node keep regressing or only moving a few percent. A fresh 2026-06-10 300k run measured generated exact-helper blog-page at 10.21 us/op, direct plan at 6.55, compact node at 6.27, and static-wave writer at 3.82. The next serious proof should generate or otherwise compile plan-specific final-owner write code for eligible raw-nested reads, with unrolled relation waves and fixed query/column/field slots. Another data-driven phase interpreter is not enough; wire Rust query-compiler emission only if an internal/benchmark path moves Node and Workerd raw-result plus generated blog-page gates clearly toward the static-wave lower bound.
