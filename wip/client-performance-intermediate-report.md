...
1. Productize descriptor-bound exact helpers behind strict internal gates and oracle tests. The first kept internal-gated slice now emits a runtime exact-helper registry for scalar `findUnique` specs supplied through `generator.config.internalExactDescriptorHelpers`. A 2026-06-09 focused Node 100k run measured generated `findUnique` default / request-precomputed / hand exact / runtime exact at 3.72 / 3.61 / 3.55 / 3.49 us/op, and batched `findUnique` at 8.31 / 8.59 / 8.08 / 8.11 us/op. The Workerd harness now imports the real runtime helper from `wasm-compiler-edge.mjs`; the confirming 20k repeat measured worker-loop request-precomputed / runtime exact at 5.35 / 4.60 us/op for `findUnique` and 9.90 / 8.85 for batched `findUnique`. Initial oracle coverage now proves exact matcher storage after slow-path self-test, exact-miss lazy fallback, exact+lazy miss slow-path fallback, string scalar support, and `undefined` / `Prisma.skip` shape rejection. This is useful groundwork, but not a default feature; the remaining product work is broader oracle coverage for duplicate placeholder reuse/order, extensions/global omit exclusions, generated-output typechecking, and more special scalar values. The broader generator-emitted proof also showed Node request-precomputed -> exact helper at 4.05 -> 3.88 us/op for `findUnique`, 9.70 -> 9.02 for batched `findUnique`, and 3.21 -> 2.95 for `findMany`; Workerd 5k host dispatch moved 10.04 -> 9.57, 14.08 -> 13.40, and 7.15 -> 5.61 respectively.
