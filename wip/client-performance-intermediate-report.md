...
- The latest compact-only internal-format cleanups removed legacy object-shaped `templateSql`, tagged fragments, and legacy query-plan PrismaValue placeholder/generator readers from the runtime/types/tests. This is smaller than the earlier memory wins, but it keeps internal hot paths version-lockstep-only. Close direct-plan timing A/Bs over 300k nested blog-page iterations measured 12.62 us/op control vs 10.93 us/op patched for `templateSql`, then 9.05 us/op control vs 8.52 us/op patched for PrismaValue after preserving the monomorphic local-object guard shape; the cache-memory probe stayed in band at 4.47 MiB for 1,000 node-warm blog-page plans and 4.69-4.70 MiB for the parameterized row.
