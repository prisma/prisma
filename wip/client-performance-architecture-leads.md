...
3. Test generated delegate/action materialization. The current generated rows still pay applied-client/model composite proxy and cached-property lookup for every `client.post.findMany(...)` before exact descriptor extraction; a useful prototype should attack that call-surface overhead without changing descriptor validation or RequestHandler semantics.
