...
3. Productize generated action hoisting only with a semantics-preserving outer/generator shape. Benchmark-only hoisted rows prove the remaining call-surface cost is real (`findUnique`, `findMany users`, and feed-by-author rows save about 0.2-0.4 us/op), but the inner model-action-target product attempt failed. The next proof should remove both model delegate and action lookup per generated call without breaking extensions, fluent APIs, transactions, or overwrite semantics.
