...
5. For Rust memory management, keep taking profile-backed slices. Do not start an arena/borrowing rewrite until the target structure and Criterion row are named. The next larger design candidate is direct placeholder storage for `Flow` / `Diff` projected inputs.
