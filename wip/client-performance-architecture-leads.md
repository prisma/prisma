...
3. Continue generated call-surface work only beyond the accepted target-cache slice. Stable model delegate/action layers now cache resolved values on the composite-proxy target after first access and skip their old `cacheProperties()` wrappers, which improved simple generated rows while keeping feed-by-author product rows roughly neutral. Benchmark-only hoisted rows still prove more call-surface cost remains, but the inner model-action-target product attempt failed. The next proof should remove more proxy trap/action lookup cost, likely through generated-code hoisting or a deeper applied-client shape, without breaking extensions, fluent APIs, transactions, or overwrite semantics.
