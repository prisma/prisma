...
- aggregation result-node data mapping in `query-compiler/query-compiler/src/data_mapper.rs::get_result_node_for_aggregation()`. The scout lead is to avoid the `IndexSet` / `ordered_set.get_index_of()` sorting path if `aggregate`, `aggregate-custom`, and `group-by` allocation profiles show it is real.
...
5. For Rust memory management, keep taking profile-backed slices. Do not start an arena/borrowing rewrite until the target structure and Criterion row are named. The next concrete Rust scout candidate is aggregation result-node mapping (`aggregate`, `aggregate-custom`, `group-by`) before any broader borrowing/arena work.
