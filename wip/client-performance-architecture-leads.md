...
- `QueryGraph.visited` capacity during translation. A 2026-06-18 scout identified `core/src/query_graph/mod.rs`'s `visited: Vec<NodeIndex>` as a concrete compile-local container: every translated node calls `mark_visited()`, but the vector starts empty. The bounded spike is to reserve `self.graph.node_count()` once before root traversal in `query-compiler/src/translate.rs`. Gate it on `translate_ir` / `full_compile` allocation-count drops for nested-write rows such as `nested-upsert-nested-only`, `update-set-nested`, and `create-nested-connectOrCreate-*`, with no count regressions on `create-nested-create`, `query-m2o`, `query-many-m2m`, or `aggregate`, and close Criterion neutral-to-positive.
- direct placeholder storage for `Flow` / `Diff` projected inputs. A 2026-06-18 scout found that `If`, `Return`, and `Diff` inputs store `Vec<SelectionResult>` even when translation immediately needs exactly one placeholder. A larger spike could add a direct placeholder/single-projection sink for `IfInput`, `ReturnInput`, `LeftSideDiffInput`, and `RightSideDiffInput`, targeting `translate_ir` on `nested-upsert-nested-only`, `update-set-nested`, and connect-or-create rows. Avoid nearby rejected standalone container rewrites: `DataRule` SmallVec, `Let.bindings` SmallVec, broad typed binding names, and direct `SelectionResult::from_selected_pairs()`.
- one-to-many nested `set` bidirectional diff fusion. The non-empty path still builds two diff computation nodes from the same old/new result sets. A worthwhile design would remove one computation node, two edges, and one diff expression with a new separately addressable internal expression output; if the internal expression shape changes, replace Rust/TS/snapshots lockstep with no old/new compatibility readers.
- branch-aware nested-upsert shared target-read proof. `nested-upsert-nested-only` reads/connects the same `Category { id: 10 }` in both branches. The already-rejected exact-match dedupe regressed allocations, so a viable follow-up needs a real branch-aware shared-node/error-order proof, not a local duplicate-read rewrite.
...
5. For Rust memory management, keep taking profile-backed slices. Do not start an arena/borrowing rewrite until the target structure and Criterion row are named. The next bounded Rust target is `QueryGraph.visited` capacity before translation; reject it if allocation-count movement is absent or if close Criterion softens target/control rows. The next larger design candidate is direct placeholder storage for `Flow` / `Diff` projected inputs.
