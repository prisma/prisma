...
3. For nested execution, pursue a larger writer/wave descriptor only if it changes ownership of a whole phase. The current unique-root raw-nested static-wave lower-bound gap is about 0.5 us/op on Node direct rows after compiled final-owner query leaves, so one-branch final-owner edits are unlikely to survive generated-client gates. The non-unique blog-feed benchmark row has a larger gap, but the real product feed plan currently compiles to compact joins (`d/l/j`), not raw-nested `n`; a 2026-06-11 non-unique final-owner patch improved only the synthetic `n` row and was reverted. A generic compiled compact `q` renderer was also rejected on the real feed rows. The next feed runtime spike should target joined-result ownership/outer mapping in the compact join product plan, or change compiler emission to a raw-nested/static owner-writer shape before measuring generated/direct rows.
