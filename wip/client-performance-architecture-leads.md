...
- one-to-many nested `set` as a larger phase-owner rewrite. The narrow `DiffBoth` object plus side-projection shape was rejected on 2026-06-18: it removed one graph computation but only moved `update-set-nested` from `translate_ir/full_compile 1061/1807` to `1064/1806` while regressing adjacent nested-write controls. A worthwhile design needs to own more of the set connect/disconnect semantics as one operation, not just expose separately addressable diff sides. The current best shape is a required-child one-to-many set operation that owns old/new reads, new-only connect/update, and old-only relation-violation validation for the single-scalar id/link case first. If the internal expression/query-plan shape changes, replace Rust/TS/snapshots lockstep with no old/new compatibility readers.
...
- one-to-many nested `set` `DiffBoth` plus side projections by itself. The 2026-06-18 spike added a fused graph computation, side-specific projected dependencies, and `Expression::DiffBoth` / `Expression::DiffSide` with no old-format readers. It saved four graph-build allocations on the target row but regressed translation and adjacent nested-write controls, so future set work needs a larger semantic operation rather than a bidirectional diff object.
...
5. For Rust memory management, keep taking profile-backed slices. Do not start an arena/borrowing rewrite until the target structure and Criterion row are named. Direct placeholder storage for `Flow` / `Diff`, identity parent-binding skipping, the nested-upsert shared m2m connect join, and singleton parser value consumption have now been accepted. Do not retry the rejected one-to-many nested `set` `DiffBoth` object plus side-projection shape or the `RowSink::Discard` field-binding skip; a future set attempt needs to remove a larger semantic phase, preferably a required-child set owner for the single-scalar id/link case first, lockstep across Rust/TS/tests with no old/new compatibility path.
