...
3. Continue generated call-surface work only beyond the accepted target-cache slice. Stable model delegate/action layers now cache resolved values on the composite-proxy target after first access and skip their old `cacheProperties()` wrappers, which improved simple generated rows while keeping feed-by-author product rows roughly neutral. Benchmark-only hoisted rows still prove more call-surface cost remains on Node and Workerd; a 20k Workerd feed-by-author focused run moved exact-helper host/worker-loop from 7.84/6.55 to 7.32/6.05 us/op when the action was hoisted. The inner model-action-target product attempt failed, and a proxy Set fast-path tweak was rejected, so the next proof should remove more proxy trap/action lookup cost, likely through generated-code hoisting or a deeper applied-client shape, without breaking extensions, fluent APIs, transactions, or overwrite semantics.
