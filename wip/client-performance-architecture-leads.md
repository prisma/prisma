...
5. For Rust memory management, keep taking profile-backed slices. Do not start an arena/borrowing rewrite until the target structure and Criterion row are named. Direct placeholder storage for `Flow` / `Diff` and identity parent-binding skipping have now been accepted; the next larger design candidate is still the fused one-to-many nested `set` bidirectional diff, but only if it introduces a named internal expression/output shape lockstep across Rust/TS/tests with no old/new compatibility path.
