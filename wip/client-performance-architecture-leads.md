...
- empty-set graph shapes. One-to-many `set: []` likely does not need a new-child read, two diff nodes, or the impossible connect branch. Add an `update-set-nested-empty` fixture before trying this so the disconnect-all semantics and required-child-side violation stay covered.
- top-level upsert empty/nested-only update branches. This may mirror the nested-upsert pass-through, but needs focused fixtures and care around result-read ordering and `GetFirstNonEmpty`.
