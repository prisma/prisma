...
3. Continue generated call-surface work only beyond the accepted target-cache slice. Stable model delegate/action layers now cache resolved values on the composite-proxy target after first access and skip their old `cacheProperties()` wrappers, which improved simple generated rows while keeping feed-by-author product rows roughly neutral. Benchmark-only hoisted rows still prove more call-surface cost remains on Node and Workerd; a 20k Workerd feed-by-author focused run moved exact-helper host/worker-loop from 7.84/6.55 to 7.32/6.05 us/op when the action was hoisted. The inner model-action-target product attempt failed, an edge-only direct-client lazy-model-property fast path regressed the 50k Workerd feed rows, and both proxy target-cache micro-fast paths were rejected: reusing `overwrittenKeys` lost close Node rows, while deleting target-cached keys from `keysToLayerMap` lost priority Workerd descriptor-bound/exact-helper feed rows. The next proof should remove more proxy trap/action lookup cost, likely through generated-code hoisting or a deeper applied-client shape, without breaking extensions, fluent APIs, transactions, or overwrite semantics.
...
5. For Rust memory management, keep taking profile-backed slices. Do not start an arena/borrowing rewrite until the target structure and Criterion row are named.
