...
describe('seed', () => {
  ...
  it('seed.js', async () => {
    ...
    expect(
      ctx.mocked['console.info'].mock.calls.join('\n'),
    ).toMatchInlineSnapshot(`Running node "prisma/seed.js" ...`)
    expect(
      ctx.mocked['console.error'].mock.calls.join('\n'),
    ).toMatchInlineSnapshot(``)
  })
  ...
  it('seed.ts', async () => {
    ...
    expect(
      ...
      `Running ts-node -p -e "declare var require: any; const __seed = require('./prisma/seed.ts'); Object.keys(__seed)?.indexOf('default') !== -1 ? __seed.default() : 'Executing as script'" ...`,
    )
    expect(
      ctx.mocked['console.error'].mock.calls.join('\n'),
    ).toMatchInlineSnapshot(``)
  ...
  it('seed.ts seed function', async () => {
    ctx.fixture('seed-sqlite')
    ctx.fs.remove('prisma/seed.js')
    // ctx.fs.remove('prisma/seed.ts')
    ctx.fs.remove('prisma/seed.sh')
    ...
    fs.write(
      'prisma/seed.ts',
      `
      // So compilation succeeds without installing @types/node
      declare var require: any
      declare var module: any
      
      function seed() {
        return 'hello from main export from seed.ts'
      }
      
      if (require.main === module) {
        console.info('hello from require from seed.ts')
      }
  ...
  it('Custom --schema', async () => {
    ...
    expect(
      ctx.mocked['console.info'].mock.calls.join('\n'),
    ).toMatchInlineSnapshot(`Running node "some-folder/seed.js" ...`)
    expect(
      ctx.mocked['console.error'].mock.calls.join('\n'),
    ).toMatchInlineSnapshot(``)
  })
  ...
  it('Custom --schema from package.json', async () => {
    ...
    expect(
      ctx.mocked['console.info'].mock.calls.join('\n'),
    ).toMatchInlineSnapshot(`Running node "custom-folder/seed.js" ...`)
    expect(
      ctx.mocked['console.error'].mock.calls.join('\n'),
    ).toMatchInlineSnapshot(``)
  })
