...
function parseStack({ callsite, renderPathRelative, printFullStack, originalMethod, onUs, showColors, isValidationError }: ErrorArgs): StackParams {
  const params = {
    callsiteStr: ':',
    prevLines: '\n',
  ...
  if (
    ...
    const neededHeight = params.lastErrorHeight + lineNumber - start
