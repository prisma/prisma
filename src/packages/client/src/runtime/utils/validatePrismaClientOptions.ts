...
const validators = {
  datasources: (options: any, datasourceNames: string[]) => {
    ...
    if (
      ...
      throw new PrismaClientValidationError(`Invalid value ${JSON.stringify(options)} for "datasources" provided to PrismaClient constructor`)
    }
    ...
    for (const [key, value] of Object.entries(options)) {
      if (!datasourceNames.includes(key)) {
        ...
        throw new PrismaClientValidationError(`Unknown datasource ${key} provided to PrismaClient constructor.${didYouMean}`)
      }
...
It should have this form: { url: "CONNECTION_STRING" }`
  ...
  errorFormat: (options: any) => {
    ...
    if (typeof options !== 'string') {
      throw new PrismaClientValidationError(`Invalid value ${JSON.stringify(options)} for "errorFormat" provided to PrismaClient constructor.`)
    }
    if (!errorFormats.includes(options as ErrorFormat)) {
      const didYouMean = getDidYouMean(options, errorFormats)
      throw new PrismaClientValidationError(`Invalid errorFormat ${options} provided to PrismaClient constructor.${didYouMean}`)
    }
  },
  log: (options: any) => {
    if (!options) {
    ...
    if (!Array.isArray(options)) {
      throw new PrismaClientValidationError(`Invalid value ${JSON.stringify(options)} for "log" provided to PrismaClient constructor.`)
    }
    ...
    function validateLogLevel(level: any) {
      if (typeof level === 'string') {
        if (!logLevels.includes(level as LogLevel)) {
          ...
          throw new PrismaClientValidationError(`Invalid log level "${level}" provided to PrismaClient constructor.${didYouMean}`)
        }
      }
    }
      ...
      const validators = {
        ...
        emit: value => {
          ...
          if (!emits.includes(value)) {
            ...
            throw new PrismaClientValidationError(`Invalid value ${JSON.stringify(value)} for "emit" in logLevel provided to PrismaClient constructor.${didYouMean}`)
          }
        }
      }
      ...
      if (option && typeof option === 'object') {
        for (const [key, value] of Object.entries(option)) {
          if (validators[key]) {
            ...
            throw new PrismaClientValidationError(`Invalid property ${key} for "log" provided to PrismaClient constructor`)
          }
        }
      }
    }
  ...
  __internal: value => {
    ...
    if (typeof value !== 'object') {
      throw new PrismaClientValidationError(`Invalid value ${JSON.stringify(value)} for "__internal" to PrismaClient constructor`)
    }
    for (const [key, value1] of Object.entries(value)) {
      if (!knownKeys.includes(key)) {
        const didYouMean = getDidYouMean(key, knownKeys)
        throw new PrismaClientValidationError(`Invalid property ${JSON.stringify(key)} for "__internal" provided to PrismaClient constructor.${didYouMean}`)
      }
    }
...
export function validatePrismaClientOptions(options: PrismaClientOptions, datasourceNames: string[]) {
  for (const [key, value] of Object.entries(options)) {
    if (!knownProperties.includes(key)) {
      ...
      throw new PrismaClientValidationError(`Unknown property ${key} provided to PrismaClient constructor.${didYouMean}`)
    }
